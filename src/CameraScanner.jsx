...
// used on a lot of pharmaceutical packaging — and more), decoded with the
// library's own JS decoder rather than the browser's native BarcodeDetector
// API (see the note below on why that's turned off).
const ALL_FORMATS = [
...
export default function CameraScanner({ onDetected, onError }) {
  ...
  useEffect(() => {
    ...
    // useBarCodeDetectorIfSupported is deliberately left off. Where the
    // browser's native BarcodeDetector API exists (notably iOS Safari,
    // which is what this app is mostly used on) it can report itself as
    // available while only reliably decoding QR codes \u2014 1D barcodes and
    // some manufacturer QR codes silently fail to decode, with no
    // fallback to the JS decoder because the library considers native
    // detection already "handled." Forcing the mature JS (ZXing-based)
    // decoder for every format is slightly slower per frame but far more
    // consistent across real packaging, which is what actually matters
    // here.
    const html5QrCode = new Html5Qrcode(elIdRef.current, { verbose: false, formatsToSupport: ALL_FORMATS, useBarCodeDetectorIfSupported: false });
    ...
    const timer = setTimeout(() => {
      ...
      html5QrCode
        .start(
          // This first argument must have exactly one key — the extra
          // resolution constraints below go in videoConstraints instead.
          { facingMode: "environment" },
          {
            ...
            aspectRatio: 1.5,
            disableFlip: true,
            // A higher requested resolution gives the decoder far more
            // pixels to work with across each bar of a barcode — the
            // single biggest lever for reliable 1D barcode reads on a
            // phone camera.
