    ...
    <div>
      <div className="tb-no-print">
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
          ...
          <div style={{ ...mono, fontSize: 11.5, color: T.slate }}>
            {products.length} product{products.length === 1 ? "" : "s"} with labels generated
            {totalLabels > 0 ? ` · ${totalLabels} barcode${totalLabels === 1 ? "" : "s"} queued to print` : ""}
          </div>
        </div>
          ...
          return Array.from({ length: count }).map((_, i) => (
            <div key={`${p.id}-${i}`} className="tb-label">
              ...
              <div className="tb-label-text">
                ...
                {count > 1 && <div className="tb-label-seq">#{i + 1} of {count}</div>}
              </div>
            </div>
          ));
        })}
      ...
      <style>{`
        ...
        @media print {
          ...
          .tb-label-seq { font-family: 'Space Mono', monospace; font-size: 6.5pt; color: #1E4FD6; margin-top: 0.5mm; }
        }
      `}</style>
    </div>
  );
