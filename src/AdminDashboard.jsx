...
import { ShieldCheck, LogOut, Save, RefreshCw, Search, Clock, XCircle, CheckCircle2, Building2 } from "lucide-react";
...
export default function AdminDashboard({ T, mono, body, adminEmail, onExitAdmin }) {
  ...
  const [statusFilter, setStatusFilter] = useState(null); // null | "pending" | "expired" | "active" | "trial"
  ...
  // Overview counts for the admin's at-a-glance summary panel.
  const stats = useMemo(() => {
    const counts = { pending: 0, expired: 0, active: 0, trial: 0 };
    businesses.forEach((b) => {
      const label = subscriptionState(b).label;
      if (label === "Expired") counts.expired += 1;
      else if (label === "Active") counts.active += 1;
      else if (label === "Trial") counts.trial += 1;
    });
    counts.pending = Object.keys(pendingReceiptByUser).length;
    return counts;
  }, [businesses, pendingReceiptByUser]);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    let list = businesses;
    if (q) list = list.filter((b) => (b.business_name || "").toLowerCase().includes(q) || (b.owner_email || "").toLowerCase().includes(q));
    if (statusFilter === "pending") list = list.filter((b) => pendingReceiptByUser[b.user_id]);
    else if (statusFilter) list = list.filter((b) => subscriptionState(b).label.toLowerCase() === statusFilter);
  ...
  }, [businesses, search, statusFilter, pendingReceiptByUser]);
  ...
  // Sets the expiry date to N months from today — e.g. the business paid
  // for 4 months, so the exact expiry date doesn't need to be worked out
  // by hand. Months are added from today's date, not from any existing
  // expiry, so this is meant for activating a fresh payment.
  ...
  const markActive = async (row) => {
    ...
    // Resetting subscription_welcomed_at to null is what makes the
    // one-time "your subscription is successful" message show up again
    // the next time this business logs in.
    const { error } = await supabase.from("settings").update({ subscription_status: "active", subscription_expires_at: editExpiry, subscription_welcomed_at: null }).eq("user_id", row.user_id);
    ...
    <div style={{ ...body, minHeight: "100vh", background: T.paper, color: T.ink }}>
      <div style={{ background: T.ink, color: T.cream, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <ShieldCheck size={18} />
          <div>
            <div style={{ ...mono, fontWeight: 700, fontSize: 14 }}><span style={{ color: T.blueSoft }}>TallyBust</span> — Admin Dashboard</div>
        ...
        {loading ? (
          ...
          <>
            <div style={{ ...mono, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: T.slate, marginBottom: 8 }}>
              Overview — tap a card to filter the list below
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 20 }}>
              {[
                { key: "pending", label: "Pending receipt confirmation", value: stats.pending, icon: Clock, color: T.amber },
                { key: "expired", label: "Expired registration", value: stats.expired, icon: XCircle, color: T.stamp },
                { key: "active", label: "Active subscriptions", value: stats.active, icon: CheckCircle2, color: T.green },
                { key: "trial", label: "On trial", value: stats.trial, icon: Building2, color: T.slate },
              ].map((c) => {
                const Icon = c.icon;
                const isOn = statusFilter === c.key;
                return (
                  <button
                    key={c.key}
                    onClick={() => setStatusFilter(isOn ? null : c.key)}
                    style={{
                      textAlign: "left", cursor: "pointer", padding: "12px 14px", borderRadius: 6,
                      background: T.cream, border: `2px solid ${isOn ? c.color : "transparent"}`,
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                      <Icon size={14} color={c.color} />
                      <span style={{ ...mono, fontSize: 10, letterSpacing: "0.04em", textTransform: "uppercase", color: T.slate }}>{c.label}</span>
                    </div>
                    <div style={{ fontSize: 24, fontWeight: 700, color: T.ink }}>{c.value}</div>
                  </button>
                );
              })}
            </div>
            {statusFilter && (
              <div style={{ ...mono, fontSize: 11, color: T.slate, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                Filtering: {statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)}
                <button onClick={() => setStatusFilter(null)} style={{ border: "none", background: "transparent", color: T.blue, cursor: "pointer", fontSize: 11, ...mono, textDecoration: "underline" }}>clear</button>
              </div>
            )}
                    ...
                    {editing === row.user_id ? (
                      <div style={{ marginTop: 10 }}>
                        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                          <span style={{ fontSize: 11.5, color: T.slate }}>Paid for:</span>
                          {[1, 2, 3, 4, 6, 12].map((m) => (
                            <button key={m} onClick={() => applyMonths(m)} style={{ padding: "5px 10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", fontSize: 11.5 }}>
                              {m} month{m === 1 ? "" : "s"}
                            </button>
                          ))}
                          <input type="number" min="1" placeholder="or N" value={editMonths} onChange={(e) => setEditMonths(e.target.value)} style={{ width: 56, padding: "5px 7px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 11.5, ...body }} />
                          <button onClick={() => editMonths && applyMonths(editMonths)} disabled={!editMonths} style={{ padding: "5px 10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: editMonths ? "pointer" : "not-allowed", fontSize: 11.5, color: T.slate }}>Set</button>
                        </div>
                        <div style={{ fontSize: 10.5, color: T.slateLight, marginBottom: 8 }}>Picking a number of months fills in the exact date below — from today, not from any existing expiry. Adjust the date directly if needed.</div>
