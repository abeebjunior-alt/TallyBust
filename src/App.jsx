...
import {
  ...
  ArrowDownCircle, ClipboardList, Plus, LogOut, Camera, List, Tag, AlertCircle, Lock, Printer, Trash2, Minus, RotateCcw, ChevronDown,
} from "lucide-react";
...
const RESPONSIVE_CSS = `
  * { box-sizing: border-box; }
  @media (max-width: 760px) {
    ...
    .tb-logo { padding: 0 8px 0 0 !important; flex-shrink: 0; width: auto !important; }
    .tb-nav-toggle-caret { display: inline-block !important; }
    .tb-nav-list { display: none !important; }
    .tb-sidebar.nav-open .tb-nav-list {
    ...
    .tb-sidebar.nav-open .tb-nav-btn { flex-direction: row !important; gap: 10px !important; padding: 11px 12px !important; font-size: 13.5px !important; }
    .tb-nav-backdrop { display: block; position: fixed; inset: 0; z-index: 40; background: rgba(10,18,32,0.35); }
    ...
    .tb-scan-btn { padding: 9px 16px !important; font-size: 12.5px !important; font-weight: 700 !important; border-radius: 6px !important; }
    .tb-user-email { display: none !important; }
    .tb-signout-label { display: none; }
    .tb-switchuser-btn { background: transparent !important; color: ${T.blueSoft} !important; border: 1px solid ${T.blueSoft} !important; padding: 6px 9px !important; font-size: 10px !important; font-weight: 600 !important; border-radius: 6px !important; }
  ...
  .tb-nav-toggle-caret { display: none; }
  .tb-nav-backdrop { display: none; }
`;
...
const ROLES = [
  ...
  { role: "Cashier", access: "Dashboard (Stock Out & Refunds) & Sales" },
  { role: "Storekeeper", access: "Dashboard (Stock In / Count) & Inventory / Labels" },
];
const ROLE_TABS = {
  Admin: ["dashboard", "inventory", "labels", "sales", "reports", "refunds", "staff", "settings"],
  ...
  return (
    <div style={{ ...body, minHeight: "100vh", background: T.ink, color: T.cream, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ maxWidth: 420, width: "100%", background: T.cream, color: T.ink, borderRadius: 8, padding: "28px 26px" }}>
        ...
        <div style={{ borderTop: `2px solid ${T.ink}`, marginTop: 20, paddingTop: 18 }}>
          <div style={{ background: "#EAF0FF", border: `1.5px solid ${T.blue}`, borderRadius: 8, padding: "16px 16px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <Receipt size={17} color={T.blue} />
              <div style={{ fontSize: 15, fontWeight: 700, color: T.ink }}>Already paid? Upload your receipt</div>
            </div>
            <div style={{ fontSize: 12.5, color: T.slate, marginBottom: 12, lineHeight: 1.4 }}>
              Attach a photo or screenshot of your payment and we'll verify it and reactivate your account.
            </div>

            {pending ? (
              <div style={{ background: T.paper, borderRadius: 6, padding: "12px 14px", fontSize: 12.5 }}>
              ...
              <form onSubmit={submit}>
                {preview ? (
                  <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10, background: T.paper, borderRadius: 6, padding: 10 }}>
                    <img src={preview} alt="Receipt preview" style={{ width: 60, height: 60, borderRadius: 4, objectFit: "cover" }} />
                    <button type="button" onClick={() => setPreview(null)} style={{ fontSize: 11.5, color: T.stamp, background: "transparent", border: "none", cursor: "pointer", fontWeight: 600 }}>Remove & choose another</button>
                  </div>
                ) : (
                  <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center", padding: "22px 14px", borderRadius: 6, border: `2px dashed ${T.blue}`, background: T.paper, cursor: "pointer", marginBottom: 10 }}>
                    <Camera size={22} color={T.blue} />
                    <span style={{ fontSize: 13, color: T.ink, fontWeight: 600 }}>Tap to attach a photo or screenshot</span>
                    <span style={{ fontSize: 11, color: T.slate }}>of your payment receipt</span>
                    <input type="file" accept="image/*" onChange={onFile} style={{ display: "none" }} />
                  </label>
                )}
                <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional) — e.g. transaction ID"
                  style={{ width: "100%", padding: "10px 10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: T.paper, fontSize: 13, ...body, outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
                <button type="submit" disabled={!preview || submitting} style={{ width: "100%", padding: "12px", borderRadius: 5, border: "none", background: preview ? T.blue : T.paperDim, color: "#fff", fontWeight: 700, cursor: preview ? "pointer" : "not-allowed", ...mono, fontSize: 13 }}>
            ...
            {toast && <div style={{ fontSize: 12, color: "#1F7A4B", marginTop: 10, fontWeight: 600 }}>{toast}</div>}
          </div>
        </div>
...
const UNLOCK_KEY = "tb_unlocked_staff";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, s) => {
      ...
      setAuthEvent(event);
      if (event === "SIGNED_IN") {
        // A fresh, interactive email+password sign-in — trust it as Owner/Admin
        // right away rather than making them pick themselves again immediately.
        setActiveStaff(OWNER_STAFF);
        sessionStorage.setItem(UNLOCK_KEY, JSON.stringify(OWNER_STAFF));
      }
      if (event === "SIGNED_OUT") {
        setActiveStaff(null);
  ...
  const unlockAs = (member) => {
    setActiveStaff(member);
    sessionStorage.setItem(UNLOCK_KEY, JSON.stringify(member));
  };
  const lock = () => {
    setActiveStaff(null);
    sessionStorage.removeItem(UNLOCK_KEY);
  ...
  if (!session) return <Login hasSession={false} onUnlock={unlockAs} />;
  if (!activeStaff) return <Login hasSession={true} userId={session.user.id} userEmail={session.user.email} onUnlock={unlockAs} />;
  return <TallyBust userId={session.user.id} userEmail={session.user.email} initialActiveStaff={activeStaff} onLock={lock} />;
}
...
function TallyBust({ userId, userEmail, initialActiveStaff, onLock }) {
  const [loading, setLoading] = useState(true);
  ...
  const startingStaff = initialActiveStaff || OWNER_STAFF;
  const [tab, setTab] = useState((ROLE_TABS[startingStaff.role] || ["dashboard"])[0]);
  const [activeStaff, setActiveStaff] = useState(startingStaff);
  ...
  const [scanUnitId, setScanUnitId] = useState(null);
  const [scanAlreadyStocked, setScanAlreadyStocked] = useState(null); // { product, status }
  const [scanCameraError, setScanCameraError] = useState("");
  const [saleCart, setSaleCart] = useState([]);
  const [receiptData, setReceiptData] = useState(null);
  const [scanRefundReason, setScanRefundReason] = useState("");
  ...
  const loadAll = useCallback(async () => {
    ...
    const [{ data: prod, error: e1 }, { data: hist, error: e2 }, { data: settingsRow, error: e3 }, { data: configRow }, { data: staffRows }, { data: refundRows }] = await Promise.all([
      ...
      supabase.from("refunds").select("*").order("created_at", { ascending: false }).limit(200),
    ]);
  ...
  const openScan = (mode) => {
    ...
    setScanMissCode(""); setScanCameraError(""); setScanOutOfStock(null); setScanRefundReason("");
    setScanUnitId(null); setScanAlreadyStocked(null);
    setSaleCart([]); setStockInLog([]);
    // Stock In / Stock Out / Refund are camera-only. Stock Count allows manual search/selection.
    setScanCameraStage(mode === "count" ? "off" : "scanning");
    setScanOpen(true);
  };
  const closeScan = () => {
    if (saleCart.length > 0 && !window.confirm(`Discard the ${saleCart.length} item${saleCart.length === 1 ? "" : "s"} already scanned for this sale?`)) return;
    setSaleCart([]); setStockInLog([]);
    setScanOpen(false);
  };
  ...
  const handleDetectedCode = async (rawCode) => {
    ...
    setScanMissCode(""); setScanOutOfStock(null); setScanAlreadyStocked(null); setScanUnitId(null);

    // Every printed label carries its own unique per-unit code — resolve
    // that first, regardless of which scan mode we're in (Stock In, Stock
    // Out, or Refund all scan the same physical sticker on the item).
    const { data: unit } = await supabase.from("stock_units").select("*").eq("id", code).maybeSingle();
    if (unit) {
      const product = products.find((p) => p.id === unit.product_id);
      if (!product) { setScanMissCode(code); return; }

      if (scanMode === "stock-in" && unit.status !== "unstocked") {
        setScanAlreadyStocked({ product, status: unit.status });
        return;
      }
      if (scanMode === "sale" && product.qty <= 0) {
        setScanOutOfStock(product);
        return;
      }
      if (scanMode === "stock-in") setScanUnitId(unit.id);
      setScanProductId(product.id);
      return;
    }

    // Fallback: legacy labels printed before per-unit tracking, which just
    // encode the plain SKU.
  ...
  const rescan = () => {
    setScanOutOfStock(null); setScanMissCode(""); setScanCameraError(""); setScanAlreadyStocked(null); setScanUnitId(null);
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanMode(newMode);
    setScanProductId(""); setScanQty(""); setScanSearch(""); setScanRefundReason("");
    setScanMissCode(""); setScanOutOfStock(null); setScanCameraError(""); setScanUnitId(null); setScanAlreadyStocked(null);
  ...
  const confirmScan = async () => {
    ...
    if (scanMode === "sale") {
      addToSaleCart(scanProduct, n);
      return;
    }

    if (scanMode === "refund") {
      await submitRefund(scanProduct, n, scanRefundReason);
      setScanOpen(false);
      return;
    }
    ...
    const { error: e2 } = await supabase.from("stock_history").insert({
      ...
      unit_price: scanProduct.purchase_price,
      staff: activeStaff.name,
      occurred_on: todayStr,
    });
    if (e1 || e2) { fireToast(`Save failed: ${(e1 || e2).message}`); return; }

    fireToast(scanMode === "stock-in" ? `Stocked in ${n} \u00d7 ${scanProduct.name}` : `Count set: ${scanProduct.name} \u2192 ${newQty}`);
    setScanOpen(false);
    loadAll();
  };

  // Stock In is scan-per-unit: each printed label is stuck on one physical
  // item, so one scan always means exactly one unit — no quantity to type.
  const confirmStockInOne = async () => {
    if (!scanProduct) return;

    // If this scan matched a specific printed unit, claim it atomically so
    // two near-simultaneous scans of the same physical label can't both win.
    if (scanUnitId) {
      const { data: claimed, error: claimErr } = await supabase
        .from("stock_units")
        .update({ status: "in_stock", stocked_in_at: new Date().toISOString() })
        .eq("id", scanUnitId)
        .eq("status", "unstocked")
        .select();
      if (claimErr) { fireToast(`Save failed: ${claimErr.message}`); return; }
      if (!claimed || claimed.length === 0) {
        fireToast(`This item was already stocked in.`);
        setScanProductId(""); setScanQty(""); setScanUnitId(null); setScanCameraStage("scanning");
        return;
      }
    }

    const newQty = scanProduct.qty + 1;
    const { error: e1 } = await supabase.from("products").update({ qty: newQty }).eq("id", scanProduct.id);
    const { error: e2 } = await supabase.from("stock_history").insert({
      user_id: userId,
      product_id: scanProduct.id,
    ...
    setAllProducts((prev) => prev.map((p) => (p.id === scanProduct.id ? { ...p, qty: newQty } : p)));
    setStockInLog((prev) => [{ id: scanProduct.id, name: scanProduct.name, newQty, key: `${scanProduct.id}-${Date.now()}` }, ...prev]);
    fireToast(`+1 \u00d7 ${scanProduct.name} \u2014 now ${newQty} in stock`);
    // loop straight back to scanning for the next physical item
    setScanProductId(""); setScanQty(""); setScanSearch(""); setScanMissCode(""); setScanOutOfStock(null); setScanCameraError(""); setScanUnitId(null); setScanAlreadyStocked(null);
    setScanCameraStage("scanning");
  };

  const submitRefund = async (product, qty, reason) => {
    const { data, error } = await supabase.from("refunds").insert({
      ...
      reason: reason || null,
      requested_by: activeStaff.name,
      status: "pending",
    }).select().single();
    if (error) { fireToast(`Could not submit refund: ${error.message}`); return; }
    setRefunds((prev) => [data, ...prev]);
    fireToast(`Refund request submitted for ${product.name} \u2014 awaiting admin approval`);
  };

  const resolveRefund = async (refund, decision) => {
    const now = new Date().toISOString();
    const { error: e1 } = await supabase.from("refunds").update({ status: decision, resolved_by: activeStaff.name, resolved_at: now }).eq("id", refund.id);
    if (e1) { fireToast(`Could not update refund: ${e1.message}`); return; }

    if (decision === "approved") {
      const product = allProducts.find((p) => p.id === refund.product_id);
      const newQty = (product?.qty || 0) + refund.qty;
      const { error: e2 } = await supabase.from("products").update({ qty: newQty }).eq("id", refund.product_id);
      const { data: newHistRow, error: e3 } = await supabase.from("stock_history").insert({
        user_id: userId,
        ...
        staff: `${activeStaff.name} (refund)`,
        occurred_on: todayStr,
      }).select().single();
      if (e2 || e3) {
        fireToast(`Refund marked approved, but stock update failed: ${(e2 || e3)?.message}`);
      } else {
        setAllProducts((prev) => prev.map((p) => (p.id === refund.product_id ? { ...p, qty: newQty } : p)));
        setHistory((prev) => [newHistRow, ...prev]);
      }
    }

    setRefunds((prev) => prev.map((r) => (r.id === refund.id ? { ...r, status: decision, resolved_by: activeStaff.name, resolved_at: now } : r)));
    fireToast(decision === "approved" ? `Refund approved \u2014 stock restored` : `Refund rejected`);
  };

  const cartQtyFor = (productId) => saleCart.filter((c) => c.id === productId).reduce((s, c) => s + c.qty, 0);

  const addToSaleCart = (product, n) => {
    const alreadyInCart = cartQtyFor(product.id);
    if (alreadyInCart + n > product.qty) {
      fireToast(`Only ${product.qty - alreadyInCart} more ${product.name} left to add`);
      return;
    }
    setSaleCart((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, sku: product.sku, qty: n, unit_price: product.selling_price }];
    });
    fireToast(`Added ${n} \u00d7 ${product.name} to sale`);
    // reset selection and go back to scanning for the next item
    setScanProductId(""); setScanQty(""); setScanSearch(""); setScanMissCode(""); setScanOutOfStock(null); setScanCameraError("");
    setScanCameraStage("scanning");
  };

  const removeFromSaleCart = (productId) => setSaleCart((prev) => prev.filter((c) => c.id !== productId));
  const updateSaleCartQty = (productId, n) => setSaleCart((prev) => prev.map((c) => (c.id === productId ? { ...c, qty: Math.max(1, Number(n) || 1) } : c)));

  const completeSale = async () => {
    if (saleCart.length === 0) return;
    const receiptNo = `RCT-${Date.now().toString(36).toUpperCase()}`;
    const results = await Promise.all(saleCart.map(async (item) => {
      const product = allProducts.find((p) => p.id === item.id);
      const newQty = Math.max(0, (product?.qty || 0) - item.qty);
      const { error: e1 } = await supabase.from("products").update({ qty: newQty }).eq("id", item.id);
      const { error: e2 } = await supabase.from("stock_history").insert({
        user_id: userId,
        product_id: item.id,
    ...
    if (failed.length) { fireToast(`Save failed: ${failed[0].message}`); return; }

    fireToast(`Sale completed \u2014 ${saleCart.length} item${saleCart.length === 1 ? "" : "s"}`);
    setReceiptData({
      receiptNo,
      items: saleCart,
      staff: activeStaff.name,
  ...
  const openReceiptForGroup = (rows) => {
    if (!rows.length) return;
    setReceiptData({
      receiptNo: rows[0].receipt_no || rows[0].id,
      items: rows.map((r) => ({ id: r.product_id, name: r.product_name, qty: r.qty, unit_price: r.unit_price })),
      staff: rows[0].staff,
      staffRole: null,
      businessName: settings.business_name,
      currency: settings.currency,
      date: new Date(rows[0].created_at || rows[0].occurred_on),
    });
  };
  ...
  const markLabelsPrinted = async (items) => {
    // items: [{ id, count }] — how many copies were just printed for each product
    const now = new Date().toISOString();
    const results = await Promise.all(items.map(({ id, count }) => {
      const p = allProducts.find((x) => x.id === id);
      const newCount = (p?.labels_printed_count || 0) + count;
      return supabase.from("products").update({ labels_printed_count: newCount, labels_printed_at: now }).eq("id", id).then(({ error }) => ({ id, newCount, error }));
    }));
    // Patch local state directly instead of a full reload — a full loadAll()
    // briefly unmounts the Labels page (shows "Loading your data…") and
    // would reset any in-progress copy-count inputs to their defaults.
    setAllProducts((prev) => prev.map((p) => {
      const hit = results.find((r) => r.id === p.id && !r.error);
      return hit ? { ...p, labels_printed_count: hit.newCount, labels_printed_at: now } : p;
    }));
  };
  ...
  return (
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className={`tb-sidebar${mobileNavOpen ? " nav-open" : ""}`} style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        <button
          className="tb-logo"
          onClick={() => setMobileNavOpen((v) => !v)}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 6px 22px", border: "none", background: "transparent", cursor: "pointer", width: "100%", textAlign: "left" }}
        >
          ...
          <div style={{ ...mono, fontWeight: 700, fontSize: 15, flex: 1 }}>TallyBust</div>
          <ChevronDown size={14} className="tb-nav-toggle-caret" style={{ transition: "transform 0.15s", transform: mobileNavOpen ? "rotate(180deg)" : "none" }} />
        </button>

        {mobileNavOpen && <div className="tb-nav-backdrop" onClick={() => setMobileNavOpen(false)} />}

        <div className="tb-nav-list" style={{ display: "contents" }}>
          {NAV.map((n) => {
            const Icon = n.icon; const active = tab === n.id;
            const showRefundBadge = n.id === "refunds" && pendingRefunds.length > 0;
            return (
              <button key={n.id} onClick={() => { setTab(n.id); setMobileNavOpen(false); }} className="tb-nav-btn" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 4, border: "none", background: active ? T.paper : "transparent", color: active ? T.ink : T.slateLight, cursor: "pointer", fontSize: 13, ...body, fontWeight: active ? 600 : 500, textAlign: "left" }}>
                ...
                {showRefundBadge && (
                  <span className="tb-nav-badge" title={`${pendingRefunds.length} refund request${pendingRefunds.length === 1 ? "" : "s"} awaiting review`} style={{ marginLeft: "auto", background: T.amber, color: "#fff", borderRadius: 999, minWidth: 16, height: 16, padding: "0 4px", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", ...mono }}>
        ...
        <div className="tb-sidebar-foot" style={{ marginTop: "auto", paddingTop: 18, borderTop: `1px solid ${T.inkSoft}` }}>
          {allowedScans.length > 0 && (
            <button onClick={() => openScan(allowedScans[0])} className="tb-scan-btn" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 10px", borderRadius: 4, border: "none", background: T.blue, color: "#fff", cursor: "pointer", fontWeight: 700, ...mono, fontSize: 12.5 }}>
          ...
          <button onClick={onLock} className="tb-switchuser-btn" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 8, padding: "8px", borderRadius: 4, border: "none", background: "transparent", color: T.slateLight, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
            <Lock size={12} /> Switch user
          </button>
          <button onClick={signOut} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 4, padding: "7px", borderRadius: 4, border: "none", background: "transparent", color: T.slateLight, cursor: "pointer", fontSize: 11 }}>
        ...
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "26px 30px 60px" }}>
          {loading ? (
            ...
            <>
              ...
              {tab === "labels" && <Labels products={products} businessName={settings.business_name} userId={userId} onAddProduct={() => setAddOpen(true)} onPrinted={markLabelsPrinted} />}
              {tab === "sales" && <Sales history={history} currency={currency} onPrintReceipt={openReceiptForGroup} />}
              {tab === "reports" && <Reports products={products} history={history} currency={currency} inventoryValue={inventoryValue} onExport={exportCSV} />}
              {tab === "refunds" && <Refunds refunds={refunds} pendingRefunds={pendingRefunds} currency={currency} onResolveRefund={resolveRefund} />}
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} candidates={scanCandidates} search={scanSearch} setSearch={setScanSearch}
          ...
          onConfirm={confirmScan} onClose={closeScan} currency={currency} allowedScans={allowedScans}
          ...
          alreadyStocked={scanAlreadyStocked}
          ...
          saleCart={saleCart} onRemoveCartItem={removeFromSaleCart} onUpdateCartQty={updateSaleCartQty} onCompleteSale={completeSale}
          refundReason={scanRefundReason} setRefundReason={setScanRefundReason}
          onConfirmStockInOne={confirmStockInOne} stockInLog={stockInLog} />
      )}
...
function Refunds({ refunds, pendingRefunds, currency, onResolveRefund }) {
  const resolved = refunds.filter((r) => r.status !== "pending").slice(0, 40);
  return (
    <div>
      <PageHeader title="Refunds" sub={`${pendingRefunds.length} pending \u00b7 ${refunds.length} total`} />

      <Panel title={`Pending${pendingRefunds.length ? ` (${pendingRefunds.length})` : ""}`} icon={RotateCcw} iconColor={T.amber}>
        {pendingRefunds.length === 0 ? (
          <div style={{ fontSize: 12.5, color: T.slate }}>No refund requests waiting for review.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {pendingRefunds.map((r) => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, background: T.paper, borderRadius: 4, padding: "10px 12px" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => onResolveRefund(r, "rejected")} style={{ padding: "6px 10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", color: T.stamp, cursor: "pointer", fontSize: 11.5, fontWeight: 600 }}>Reject</button>
                  <button onClick={() => onResolveRefund(r, "approved")} style={{ padding: "6px 10px", borderRadius: 4, border: "none", background: T.green, color: "#fff", cursor: "pointer", fontSize: 11.5, fontWeight: 600 }}>Approve</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Panel title="History">
        {resolved.length === 0 ? (
          <div style={{ fontSize: 12.5, color: T.slate }}>Resolved refunds will show up here.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {resolved.map((r) => (
              <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12.5, padding: "6px 0", borderBottom: `1px solid ${T.paperDim}` }}>
                <div>
                  <span style={{ fontWeight: 600 }}>{r.qty} \u00d7 {r.product_name}</span>
                  <span style={{ color: T.slate }}> \u00b7 requested by {r.requested_by}</span>
                </div>
                <span style={{ ...mono, fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: r.status === "approved" ? T.green : T.stamp }}>{r.status}</span>
              </div>
            ))}
          </div>
        )}
...
------------------------------------------------------------------*/
function Sales({ history, currency, onPrintReceipt }) {
  ...
  // Group rows that share a receipt_no into one transaction; ungrouped (legacy) rows stand alone.
  const groups = useMemo(() => {
    const byReceipt = new Map();
    const standalone = [];
    sales.forEach((h) => {
      if (h.receipt_no) {
        if (!byReceipt.has(h.receipt_no)) byReceipt.set(h.receipt_no, []);
        byReceipt.get(h.receipt_no).push(h);
      } else {
        standalone.push([h]);
      }
    });
    const grouped = [...byReceipt.values(), ...standalone];
    grouped.sort((a, b) => new Date(b[0].created_at || b[0].occurred_on) - new Date(a[0].created_at || a[0].occurred_on));
    return grouped;
  }, [sales]);

  return (
    <div>
      <PageHeader title="Sales" sub={`${sales.length} items sold across ${groups.length} transactions`} />
      ...
      <div className="tb-table-scroll" style={{ background: T.cream, borderRadius: 4, overflow: "hidden" }}>
       <div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr 0.6fr 0.9fr 0.9fr 0.9fr 0.5fr", padding: "10px 16px", ...mono, fontSize: 10.5, letterSpacing: "0.06em", textTransform: "uppercase", color: T.slate, borderBottom: `1px solid ${T.paperDim}` }}>
          <span>Date</span><span>Product</span><span>Qty</span><span>Unit Price</span><span>Total</span><span>Staff</span><span></span>
        </div>
        {groups.slice(0, 40).map((group) => (
          group.map((h, i) => (
            <div key={h.id} style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr 0.6fr 0.9fr 0.9fr 0.9fr 0.5fr", padding: "9px 16px", fontSize: 12.5, borderBottom: `1px solid ${T.paperDim}`, background: group.length > 1 ? "rgba(30,79,214,0.03)" : "transparent" }}>
              <span style={{ ...mono, color: T.slate }}>{i === 0 ? h.occurred_on : ""}</span>
              ...
              <span style={{ color: T.slate }}>{i === 0 ? h.staff : ""}</span>
              <span>
                {i === 0 && (
                  <button onClick={() => onPrintReceipt(group)} title="Print receipt" style={{ border: "none", background: "transparent", cursor: "pointer", color: T.slate, display: "flex" }}>
                    <Printer size={14} />
                  </button>
                )}
              </span>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {staffList.map((s) => (
            <button key={s.id} onClick={() => chooseChip(s)} style={{ padding: "7px 14px", borderRadius: 4, border: `1px solid ${activeStaff.id === s.id ? T.ink : T.paperDim}`, background: activeStaff.id === s.id ? T.ink : "transparent", color: activeStaff.id === s.id ? T.paper : T.ink, cursor: "pointer", fontSize: 12.5, fontWeight: 600 }}>
              {s.name} <span style={{ opacity: 0.65, fontWeight: 500 }}>· {roleLabel(s.role)}</span>
            </button>
          ))}
        </div>
        <div style={{ fontSize: 12, color: T.slate, marginTop: 10 }}>
          Currently acting as <strong>{activeStaff.name}</strong> ({roleLabel(activeStaff.role)}). Every scan and adjustment is recorded against them — switching to a team member requires their PIN.
        </div>
      </Panel>
      ...
      <Panel title="Team Members">
        ...
        {staffList.map((s) => (
          <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${T.paperDim}` }}>
            <div><span style={{ fontWeight: 600, fontSize: 13 }}>{s.name}</span> <span style={{ fontSize: 12, color: T.slate }}>— {roleLabel(s.role)}</span></div>
      ...
      <Panel title="Roles &amp; Access">
        {ROLES.map((r, i) => (
          <div key={r.role} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: i < ROLES.length - 1 ? `1px solid ${T.paperDim}` : "none" }}>
            <span style={{ fontWeight: 600, fontSize: 13 }}>{roleLabel(r.role)}</span>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <form onSubmit={submit} style={{ position: "relative", width: 360, background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        <Field label="Username"><input required value={name} onChange={(e) => setName(e.target.value)} style={inputStyle} placeholder="e.g. amina" /></Field>
        ...
        <Field label="Role">
          <select value={role} onChange={(e) => setRole(e.target.value)} style={inputStyle}>
            {ROLES.filter((r) => r.role !== "Admin").map((r) => <option key={r.role} value={r.role}>{roleLabel(r.role)}</option>)}
          </select>
        </Field>
...
function SettingsPane({ settings, onSave, products, deletedProducts, onDeleteProduct, onRestoreProduct }) {
  ...
  return (
    <div>
      ...
      <div style={{ height: 16 }} />
      <Panel title="Device Lock">
        <div style={{ fontSize: 12, color: T.slate, marginBottom: 10 }}>
          Set a PIN for Owner / Admin so anyone picking "Owner / Admin" on the front screen needs it —
          the same way your staff already need their own PIN. Leave blank to skip locking the Admin tile.
        </div>
        <Field label="Admin PIN (4–6 digits)">
          <input inputMode="numeric" pattern="\d{4,6}" value={local.admin_pin || ""} onChange={(e) => setLocal({ ...local, admin_pin: e.target.value.replace(/\D/g, "") })} style={{ ...inputStyle, width: 140 }} placeholder="Optional" />
        </Field>
        <button onClick={() => onSave(local)} style={{ padding: "9px 16px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, fontWeight: 700, cursor: "pointer", ...mono, fontSize: 12 }}>SAVE</button>
      </Panel>
...
function ScanModal({
  ...
  saleCart, onRemoveCartItem, onUpdateCartQty, onCompleteSale, refundReason, setRefundReason,
  onConfirmStockInOne, stockInLog, alreadyStocked,
}) {
  const ALL_MODES = [
    { id: "stock-in", label: "Stock In", color: T.green },
    { id: "sale", label: "Stock Out", color: T.stamp },
    { id: "count", label: "Stock Count", color: T.ink },
    { id: "refund", label: "Refund", color: T.amber },
  ];
  const MODES = ALL_MODES.filter((m) => allowedScans.includes(m.id));
  const cameraOn = cameraStage !== "off";
  const manualAllowed = mode === "count"; // Stock In / Stock Out / Refund are barcode-only
  const cartTotal = (saleCart || []).reduce((s, c) => s + c.qty * Number(c.unit_price), 0);
  const cartCount = (saleCart || []).reduce((s, c) => s + c.qty, 0);
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(20,20,15,0.55)" }} />
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        {mode === "stock-in" && stockInLog && stockInLog.length > 0 && (
          <div style={{ background: T.paper, borderRadius: 6, padding: 12, marginBottom: 14 }}>
            <div style={{ ...mono, fontSize: 10.5, textTransform: "uppercase", letterSpacing: "0.05em", color: T.slate, marginBottom: 8 }}>
              Scanned in this session — {stockInLog.length} item{stockInLog.length === 1 ? "" : "s"}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5, maxHeight: 140, overflow: "auto" }}>
              {stockInLog.map((entry) => (
                <div key={entry.key} style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5 }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6 }}><Check size={12} color={T.green} /> {entry.name}</span>
                  <span style={{ ...mono, color: T.slate }}>{"\u2192"} {entry.newQty} in stock</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {mode === "sale" && saleCart && saleCart.length > 0 && (
          <div style={{ background: T.paper, borderRadius: 6, padding: 12, marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ ...mono, fontSize: 10.5, textTransform: "uppercase", letterSpacing: "0.05em", color: T.slate }}>This sale — {cartCount} item{cartCount === 1 ? "" : "s"}</div>
              <div style={{ ...mono, fontSize: 13, fontWeight: 700 }}>{currency}{cartTotal.toLocaleString()}</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
              {saleCart.map((item) => (
                <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
                  ...
                  <button onClick={() => onUpdateCartQty(item.id, item.qty - 1)} style={{ width: 20, height: 20, borderRadius: 3, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Minus size={11} /></button>
                  <span style={{ ...mono, width: 22, textAlign: "center" }}>{item.qty}</span>
                  <button onClick={() => onUpdateCartQty(item.id, item.qty + 1)} style={{ width: 20, height: 20, borderRadius: 3, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Plus size={11} /></button>
                  <span style={{ ...mono, fontSize: 12, width: 60, textAlign: "right" }}>{currency}{(item.qty * Number(item.unit_price)).toLocaleString()}</span>
                  <button onClick={() => onRemoveCartItem(item.id)} style={{ border: "none", background: "transparent", cursor: "pointer", color: T.stamp, padding: 2 }}><Trash2 size={13} /></button>
                </div>
              ))}
            </div>
            <button onClick={onCompleteSale} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "11px", borderRadius: 4, border: "none", background: T.stamp, color: "#fff", cursor: "pointer", fontSize: 12.5, fontWeight: 700, ...mono }}>
              <Printer size={14} /> COMPLETE SALE & PRINT RECEIPT
            </button>
          </div>
        )}
        ...
        {!manualAllowed && (
          <div style={{ ...mono, fontSize: 10.5, color: T.slate, marginBottom: 10 }}>
            {mode === "stock-in" ? "Stock In" : mode === "refund" ? "Refund" : "Stock Out"} is barcode-only — scan the product's sticker to continue.
          </div>
        )}
        {outOfStock ? (
          ...
          <div style={{ textAlign: "center", padding: "18px 10px" }}>
            <AlertCircle size={26} color={T.amber} style={{ marginBottom: 10 }} />
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>
              {alreadyStocked.status === "sold" ? "This item was already sold" : "This item is already stocked in"}
            </div>
            <div style={{ fontSize: 12.5, color: T.slate, marginBottom: 16 }}>
              {alreadyStocked.product.name} \u2014 this exact label has {alreadyStocked.status === "sold" ? "already been sold. Scanning it again isn't expected \u2014 make sure you're scanning a fresh label." : "already been stocked in once. Scanning the same label twice would count it as two items by mistake."}
            </div>
            <button onClick={onRescan} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, cursor: "pointer", fontSize: 12.5, fontWeight: 600, ...mono }}>
              <Camera size={14} /> SCAN ANOTHER
            </button>
          ...
          <div>
            ...
            <div style={{ fontSize: 12.5, color: T.slate, marginBottom: 14, textAlign: "center" }}>
              This label is for <strong>one</strong> unit. Confirming adds exactly 1 to stock — scan the next label for the next unit.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={onConfirmStockInOne} style={{ flex: 2, padding: "10px", borderRadius: 4, border: "none", background: T.green, color: T.cream, cursor: "pointer", fontSize: 12.5, fontWeight: 700, ...mono }}>
                CONFIRM & SAVE (+1)
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <Field label={mode === "count" ? "Counted Quantity" : mode === "refund" ? "Quantity Returned" : "Quantity Sold"}>
              <input type="number" min="0" autoFocus value={qty} onChange={(e) => setQty(e.target.value)} placeholder="0" style={inputStyle} />
            </Field>
            {mode === "sale" && qty && <div style={{ fontSize: 12.5, color: T.slate, marginBottom: 10 }}>Line total: {currency}{(Number(qty) * selected.selling_price).toLocaleString()}</div>}
            {mode === "refund" && (
              <Field label="Reason (optional)">
                <input value={refundReason} onChange={(e) => setRefundReason(e.target.value)} placeholder="e.g. customer returned damaged item" style={inputStyle} />
              </Field>
            )}
            <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
              <button onClick={() => setSelected("")} style={{ flex: 1, padding: "10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", fontSize: 12.5, fontWeight: 600 }}>Back</button>
              <button onClick={onConfirm} disabled={!qty} style={{ flex: 2, padding: "10px", borderRadius: 4, border: "none", background: qty ? (mode === "refund" ? T.amber : T.ink) : T.slateLight, color: T.cream, cursor: qty ? "pointer" : "not-allowed", fontSize: 12.5, fontWeight: 700, ...mono }}>
                {mode === "sale" ? "ADD TO SALE" : mode === "refund" ? "SUBMIT REFUND REQUEST" : "CONFIRM COUNT"}
              </button>
            </div>
            {mode === "sale" && (
              <div style={{ fontSize: 11, color: T.slate, marginTop: 8, textAlign: "center" }}>Keep scanning to add more items, then complete the sale to print one receipt.</div>
            )}
            {mode === "refund" && (
              <div style={{ fontSize: 11, color: T.slate, marginTop: 8, textAlign: "center" }}>Stock won't change until an admin approves this request.</div>
            )}
          </div>
        )}
      </div>
...
function ReceiptModal({ data, onClose }) {
  const { receiptNo, items, staff, staffRole, businessName, currency, date } = data;
  const total = items.reduce((s, it) => s + it.qty * Number(it.unit_price), 0);
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div className="tb-no-print" onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(20,20,15,0.6)" }} />
      <div className="tb-no-print" style={{ position: "relative", width: 340, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}><Receipt size={16} /><h2 style={{ fontSize: 15, fontWeight: 700, margin: 0 }}>Sale Complete</h2></div>
          <button onClick={onClose} style={{ border: "none", background: "transparent", cursor: "pointer" }}><X size={17} /></button>
        </div>

        <div style={{ background: T.paper, borderRadius: 4, padding: 14, marginBottom: 14 }}>
          <div style={{ textAlign: "center", marginBottom: 10 }}>
            <div style={{ fontWeight: 700, fontSize: 14 }}>{businessName}</div>
            <div style={{ ...mono, fontSize: 10, color: T.slate, marginTop: 2 }}>Receipt {receiptNo}</div>
            <div style={{ ...mono, fontSize: 10, color: T.slate }}>{date.toLocaleString()}</div>
            {staff && <div style={{ fontSize: 11, color: T.slate, marginTop: 2 }}>Served by {staff}{staffRole ? ` (${roleLabel(staffRole)})` : ""}</div>}
          </div>
          <div style={{ borderTop: `1px dashed ${T.paperDim}`, borderBottom: `1px dashed ${T.paperDim}`, padding: "8px 0", marginBottom: 8 }}>
            {items.map((it) => (
              <div key={it.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "3px 0" }}>
                <span style={{ flex: 1 }}>{it.name} <span style={{ color: T.slate }}>x{it.qty}</span></span>
                <span style={{ ...mono }}>{currency}{(it.qty * Number(it.unit_price)).toLocaleString()}</span>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 700 }}>
            <span>TOTAL</span>
            <span style={{ ...mono }}>{currency}{total.toLocaleString()}</span>
          </div>
        </div>

        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: "10px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", fontSize: 12.5, fontWeight: 600 }}>Close</button>
          <button onClick={() => window.print()} style={{ flex: 2, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, cursor: "pointer", fontSize: 12.5, fontWeight: 700, ...mono }}>
            <Printer size={14} /> PRINT RECEIPT
          </button>
        </div>
      </div>

      {/* print-only receipt slip */}
      <div className="tb-receipt-print-sheet">
        <div className="tb-receipt-biz">{businessName}</div>
        <div className="tb-receipt-meta">Receipt {receiptNo}</div>
        <div className="tb-receipt-meta">{date.toLocaleString()}</div>
        {staff && <div className="tb-receipt-meta">Served by {staff}{staffRole ? ` (${roleLabel(staffRole)})` : ""}</div>}
        <div className="tb-receipt-divider" />
        {items.map((it) => (
          <div key={it.id} className="tb-receipt-line">
            <span>{it.name} x{it.qty}</span>
            <span>{currency}{(it.qty * Number(it.unit_price)).toLocaleString()}</span>
          </div>
        ))}
        <div className="tb-receipt-divider" />
        <div className="tb-receipt-total">
          <span>TOTAL</span>
      ...
      <style>{`
        .tb-receipt-print-sheet { display: none; }
        @media print {
          body * { visibility: hidden; }
          .tb-no-print { display: none !important; }
          .tb-receipt-print-sheet, .tb-receipt-print-sheet * { visibility: visible; }
          .tb-receipt-print-sheet {
            display: block;
            position: absolute;
            top: 0; left: 0;
            width: 78mm;
          ...
          .tb-receipt-biz { font-size: 12pt; font-weight: 700; text-align: center; }
          .tb-receipt-meta { font-size: 8pt; text-align: center; margin-top: 1mm; }
          .tb-receipt-divider { border-top: 1px dashed #000; margin: 3mm 0; }
          .tb-receipt-line { display: flex; justify-content: space-between; font-size: 9pt; padding: 0.8mm 0; }
          .tb-receipt-total { display: flex; justify-content: space-between; font-size: 11pt; font-weight: 700; }
        }
      `}</style>
    </div>
  );
