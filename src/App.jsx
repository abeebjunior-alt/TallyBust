...
export default function App() {
  ...
  const [identity, setIdentity] = useState(null); // null = not yet chosen | "owner" | a staff record
  ...
  if (!session || !identity) {
    return (
      <Login
        hasSession={!!session}
        onOwnerAuthenticated={() => setIdentity("owner")}
        onStaffIdentified={(staff) => setIdentity(staff)}
      />
    );
  }
  return (
    <TallyBust
      ...
      initialActiveStaff={identity === "owner" ? OWNER_STAFF : identity}
    ...
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className="tb-sidebar" style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div className="tb-sidebar-foot" style={{ marginTop: "auto", paddingTop: 18, borderTop: `1px solid ${T.inkSoft}` }}>
          ...
          {onLock && (
            <button onClick={onLock} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 6, padding: "7px", borderRadius: 4, border: "none", background: "transparent", color: T.slateLight, cursor: "pointer", fontSize: 11 }}>
              <Lock size={12} /> <span className="tb-signout-label">Switch user</span>
            </button>
          )}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <form onSubmit={submit} style={{ position: "relative", width: 360, background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        <div style={{ fontSize: 11, color: T.slate, marginBottom: 12, marginTop: -6 }}>They'll use this username and PIN to log in on the Staff login screen.</div>
        <Field label="Role">
