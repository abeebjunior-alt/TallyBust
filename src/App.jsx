...
function TallyBust({ userId, userEmail, initialActiveStaff, onLock }) {
  ...
  const openScan = (mode) => {
    ...
    // Stock In / Stock Out are camera-only. Stock Count and Refund allow manual search/selection.
    setScanCameraStage(mode === "count" || mode === "refund" ? "off" : "scanning");
    setScanOpen(true);
  };
  const closeScan = () => {
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanCameraStage(newMode === "count" || newMode === "refund" ? "off" : "scanning");
  };
        ...
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "26px 30px 60px" }}>
          {loading ? (
            ...
            <>
              ...
              {tab === "labels" && <Labels products={products} businessName={settings.business_name} onAddProduct={() => setAddOpen(true)} onPrinted={markLabelsPrinted} />}
...
function ScanModal({
  ...
  onConfirmStockInOne, stockInLog,
}) {
  const ALL_MODES = [
  ...
  const manualAllowed = mode === "count" || mode === "refund"; // Stock In / Stock Out are barcode-only
