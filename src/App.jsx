...
import {
  ...
  BookOpen, Phone, Pencil, Trash2, Bell,
} from "lucide-react";
...
const ROLE_TABS = {
  Admin: ["dashboard", "inventory", "labels", "sales", "debit-sales", "refunds", "stock-health", "reports", "staff", "settings"],
...
// ---- Debit Sales helpers ----
// The owner is reminded starting this many days before a payment date, and
// every day after it until the debt is marked paid or given a new date.
const DEBT_REMIND_DAYS_BEFORE = 1;
// Local calendar date (YYYY-MM-DD). Deliberately not iso(): that one is
// UTC-based, so in Nigeria (UTC+1) it would still say "yesterday" for the
// first hour after midnight.
const localDateStr = (d = new Date()) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
const fmtDebtDate = (s) => (s ? new Date(`${s}T00:00:00`).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "");
function debtDueState(d, nowStr) {
  if (d.status === "paid") return { key: "paid" };
  if (!d.due_date) return { key: "nodate" };
  const n = Math.round((Date.parse(`${d.due_date}T00:00:00Z`) - Date.parse(`${nowStr}T00:00:00Z`)) / 86400000);
  if (n < 0) return { key: "overdue", days: -n };
  if (n === 0) return { key: "today" };
  if (n <= DEBT_REMIND_DAYS_BEFORE) return { key: "soon", days: n };
  return { key: "upcoming", days: n };
}
const debtNeedsAttention = (d, nowStr) => ["overdue", "today", "soon"].includes(debtDueState(d, nowStr).key);
...
// A brand-new account has no real business name yet: the settings row is
// created with a placeholder. Until the Owner replaces it, they're held on a
// "Name your business" screen (see BusinessNameGate). Add other placeholder
// values here if your settings table's default is different.
const PLACEHOLDER_BUSINESS_NAMES = ["", "my business"];
const isPlaceholderBusinessName = (name) => PLACEHOLDER_BUSINESS_NAMES.includes((name || "").trim().toLowerCase());

function BusinessNameGate({ onSave, onSignOut }) {
  ...
  const submit = async (e) => {
    ...
    if (isPlaceholderBusinessName(clean)) { setError("Please enter your actual business name."); return; }
    setError(""); setSaving(true);
    const msg = await onSave(clean); // returns an error message, or "" on success
    setSaving(false);
    if (msg) setError(msg);
  };

  return (
    <div style={{ ...body, minHeight: "100vh", background: T.ink, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <link rel="preconnect" href="https://fonts.googleapis.com" />
      <link href="https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&family=Work+Sans:wght@400;500;600;700&display=swap" rel="stylesheet" />
      <form onSubmit={submit} style={{ width: 360, maxWidth: "100%", background: T.cream, borderRadius: 6, padding: 26 }}>
        <div style={{ ...mono, fontWeight: 700, fontSize: 15, color: T.blue, marginBottom: 14 }}>TallyBust</div>
        <h1 style={{ ...body, fontSize: 20, fontWeight: 700, margin: 0, color: T.ink }}>Name your business</h1>
        <div style={{ fontSize: 12.5, color: T.slate, margin: "6px 0 16px", lineHeight: 1.45 }}>
          This is the name on your dashboard, labels and receipts. You can change it later in Settings.
        </div>
        <Field label="Business name">
          <input required autoFocus maxLength={60} value={name} onChange={(e) => setName(e.target.value)} style={inputStyle} placeholder="e.g. Al-Amin Pharmacy" />
        </Field>
        {error && <div style={{ fontSize: 12, color: T.stamp, marginBottom: 10 }}>{error}</div>}
        <button type="submit" disabled={saving} style={{ width: "100%", padding: "10px", borderRadius: 4, border: "none", background: T.blue, color: "#fff", fontWeight: 700, cursor: saving ? "default" : "pointer", opacity: saving ? 0.7 : 1, ...mono, fontSize: 12.5 }}>
          {saving ? "SAVING…" : "SAVE & CONTINUE"}
        </button>
        <button type="button" onClick={onSignOut} style={{ width: "100%", marginTop: 10, padding: 6, border: "none", background: "transparent", color: T.slate, cursor: "pointer", fontSize: 11.5, ...body }}>Sign out</button>
      </form>
    </div>
  );
}
...
function TallyBust({ userId, userEmail, initialActiveStaff }) {
  ...
  const [debitSales, setDebitSales] = useState([]); // record-keeping only — never feeds stock, sales, or reports
  const [debtBannerHidden, setDebtBannerHidden] = useState(false);
  ...
  const loadAll = useCallback(async () => {
    ...
    const [{ data: prod, error: e1 }, { data: hist, error: e2 }, { data: settingsRow, error: e3 }, { data: configRow }, { data: staffRows }, { data: refundRows }, { data: writeOffRows }, { data: unitRows }, { data: debitRows }] = await Promise.all([
      ...
      supabase.from("debit_sales").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(1000),
    ]);
  ...
  /* ---------- debit sales (record-keeping only) ----------
     These four functions only ever read/write the debit_sales table.
     They never touch products, stock_history, product_units, refunds, or
     anything the Dashboard / Sales / Reports figures are built from. */
  const saveDebitSale = async (form, existingId) => {
    const row = {
    ...
    const { error } = existingId
      ? await supabase.from("debit_sales").update(row).eq("id", existingId).eq("user_id", userId)
      : await supabase.from("debit_sales").insert({ ...row, user_id: userId, recorded_by: activeStaff.name, status: "pending" });
    if (error) { fireToast(`Could not save: ${error.message}`); return false; }
    fireToast(existingId ? "Debit record updated" : `Debit sale recorded for ${row.debtor_name}`);
    loadAll();
    return true;
  };

  const setDebitPaid = async (debt, paid) => {
    const { error } = await supabase.from("debit_sales")
      .update({ status: paid ? "paid" : "pending", paid_on: paid ? localDateStr() : null })
      .eq("id", debt.id).eq("user_id", userId);
    if (error) { fireToast(`Could not update: ${error.message}`); return; }
    fireToast(paid ? `${debt.debtor_name} marked as paid` : `${debt.debtor_name} moved back to unpaid`);
    loadAll();
  };

  const deleteDebitSale = async (debt) => {
    if (!window.confirm(`Delete the debit record for ${debt.debtor_name} (${debt.item_name})? This can't be undone.`)) return;
    const { error } = await supabase.from("debit_sales").delete().eq("id", debt.id).eq("user_id", userId);
    if (error) { fireToast(`Could not delete: ${error.message}`); return; }
    fireToast("Debit record deleted");
    loadAll();
  };
  ...
  // Registers a genuinely brand-new item right from the scan flow, using
  // the manufacturer's own scanned code as its permanent stock code \u2014
  // no detour through the separate Add Item modal, no printing a fresh
  // TallyBust label first. The moment it's saved it's pushed into the
  // scan pile via linkExternalCode, so from here on this code lives in
  // product_units exactly like a TallyBust-generated one: same table,
  // same fields, so stock-out, refunds, and write-offs all treat it
  // identically \u2014 there's nothing elsewhere that needs to know which
  // system originally produced the code.
  const quickAddAndStockNewProduct = async (rawCode, form) => {
    ...
    if (error) { fireToast(`Could not add item: ${error.message}`); return; }
    loadAll(); // syncs Inventory/Reports with the new product in the background
    linkExternalCode(code, data);
  };
  ...
  const handleDetectedCode = async (rawCode) => {
    ...
    // Re-scanning a code that's already sitting in this session's pile —
    // added, but not yet committed by tapping Accept/Mark Sold — used to
    // fall straight through to "isn't one of ours yet" below, since
    // product_units has no row for it until Accept actually runs. That
    // restarted the whole "which item is this?" flow for something the
    // person had already just told the app about, which looked like the
    // item (and the Accept button under it) had vanished. Catching it
    // here instead gives a direct, honest answer: it's already queued,
    // finish the batch to make it real.
    const pileMatch = scanPile.find((it) => it.code.toLowerCase() === trimmedCode.toLowerCase());
    if (pileMatch) {
      setScanUnitError(`${pileMatch.product.name} (${trimmedCode}) is already in this batch \u2014 tap ${scanMode === "sale" ? "\"Mark Sold\"" : "ACCEPT"} below to finish.`);
  ...
  const confirmStockInBatch = async () => {
    ...
    let successCount = 0;
    const failures = []; // a DB error here used to be silently swallowed \u2014
    // the item just vanished from the pile with no explanation, and the
    // code was never actually stocked in, so every later scan of it kept
    // saying "not recognized" with no clue why.
    const stillFailed = []; // failed items stay in the pile instead of
    // disappearing along with the successful ones, so nothing has to be
    // re-scanned from scratch just because one item in the batch failed.
    for (const item of scanPile) {
      ...
      if (item.unitId) {
        ...
        if (error) { ok = false; failures.push(`${item.product.name} (${item.code}): ${error.message}`); }
      } else {
      ...
      if (!ok) { stillFailed.push(item); continue; }
    ...
    if (failures.length > 0) {
      fireToast(`${successCount} accepted, ${failures.length} failed: ${failures[0]}`);
    } else {
      fireToast(`${successCount} item${successCount === 1 ? "" : "s"} accepted into stock`);
    }
    setScanPile(stillFailed);
    if (stillFailed.length === 0) setScanOpen(false);
  ...
  const confirmStockOutBatch = async () => {
    ...
    for (const item of scanPile) {
      ...
      if (error) { failures.push(`${item.product.name} (${item.code}): ${error.message}`); stillFailed.push(item); continue; }
      const currentQty = qtyMap.has(item.product.id) ? qtyMap.get(item.product.id) : item.product.qty;
    ...
    if (failures.length > 0) {
      fireToast(`${successCount} sold, ${failures.length} failed: ${failures[0]}`);
    } else {
  ...
  // First-login name prompt. Only writes business_name; returns an error
  // message (shown on the gate screen) or "" on success.
  const saveBusinessName = async (name) => {
    const { error } = await supabase.from("settings").update({ business_name: name }).eq("user_id", userId);
    if (error) return `Could not save: ${error.message}`;
    setSettings((s) => ({ ...s, business_name: name }));
    return "";
  };

  const saveSettings = async (next) => {
    // Editing later in Settings is fine, but the name can't be blanked out
    // (that would just send them back to the first-login prompt).
    if (isPlaceholderBusinessName(next.business_name)) { fireToast("Business name can't be empty."); return; }
    next = { ...next, business_name: next.business_name.trim() };
    setSettings(next);
  ...
  // A staff-submitted refund or write-off just sat quietly in its own page
  // until the Owner/Admin happened to click into it \u2014 nothing in the nav
  // itself signaled that something was waiting on their approval.
  const pendingRefundCount = useMemo(() => refunds.filter((r) => r.status === "pending").length, [refunds]);
  const pendingWriteOffCount = useMemo(() => (writeOffs || []).filter((w) => w.status === "pending").length, [writeOffs]);

  // Debit payments the owner should follow up on: overdue, due today, or due
  // within DEBT_REMIND_DAYS_BEFORE days. Only counted for roles that can
  // actually see the Debit Sales tab.
  const debtNowStr = localDateStr();
  const debtCounts = useMemo(() => {
    const c = { overdue: 0, today: 0, soon: 0 };
    if (!allowedTabs.includes("debit-sales")) return c;
    debitSales.forEach((d) => { const k = debtDueState(d, debtNowStr).key; if (k in c) c[k] += 1; });
    return c;
  }, [debitSales, debtNowStr, allowedTabs]);
  const debtAttentionCount = debtCounts.overdue + debtCounts.today + debtCounts.soon;
  const navAttentionCount = pendingRefundCount + pendingWriteOffCount + debtAttentionCount;
  ...
  const NAV = [
    ...
    { id: "debit-sales", label: "Debit Sales", icon: BookOpen, badge: debtAttentionCount },
    { id: "refunds", label: "Refunds", icon: RotateCcw, badge: pendingRefundCount + pendingWriteOffCount },
  ...
  // First login after creating an account: the Owner must name the business
  // before anything else is reachable. Not shown to staff logging in on their
  // own device (the Owner names it) or to platform admins.
  if (!loading && !isAdmin && activeStaff.id === null && isPlaceholderBusinessName(settings.business_name)) {
    return <BusinessNameGate onSave={saveBusinessName} onSignOut={signOut} />;
  }
    ...
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className="tb-sidebar" style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        <div className="tb-logo" onClick={() => setMobileNavOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 6px 22px" }}>
          ...
          />
          {!mobileNavOpen && navAttentionCount > 0 && (
            <span style={{ marginLeft: "auto", width: 8, height: 8, borderRadius: "50%", background: T.stamp }} title={`${navAttentionCount} need attention`} />
          )}
        </div>
          ...
          {NAV.map((n) => {
            ...
            return (
              <button key={n.id} onClick={() => { setTab(n.id); setMobileNavOpen(false); }} className="tb-nav-btn" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 4, border: "none", background: active ? T.paper : "transparent", color: active ? T.ink : T.slateLight, cursor: "pointer", fontSize: 13, ...body, fontWeight: active ? 600 : 500, textAlign: "left" }}>
                ...
                {!!n.badge && (
                  <span style={{ marginLeft: "auto", background: T.stamp, color: "#fff", borderRadius: 999, minWidth: 17, height: 17, padding: "0 5px", fontSize: 10.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", ...mono }}>
                    {n.badge}
                  </span>
                )}
              </button>
      ...
      <div className="tb-content" style={{ flex: 1, background: T.paper, overflow: "auto" }}>
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "26px 30px 60px" }}>
          {!loading && debtAttentionCount > 0 && tab !== "debit-sales" && !debtBannerHidden && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: T.cream, borderLeft: `3px solid ${debtCounts.overdue + debtCounts.today > 0 ? T.stamp : T.amber}`, borderRadius: 4, padding: "10px 12px", marginBottom: 16 }}>
              <Bell size={15} color={debtCounts.overdue + debtCounts.today > 0 ? T.stamp : T.amber} style={{ flexShrink: 0 }} />
              <button onClick={() => setTab("debit-sales")} style={{ flex: 1, textAlign: "left", background: "transparent", border: "none", cursor: "pointer", ...body, fontSize: 12.5, color: T.ink, padding: 0 }}>
                <strong>Debit payments to follow up:</strong>{" "}
                {[debtCounts.overdue && `${debtCounts.overdue} overdue`, debtCounts.today && `${debtCounts.today} due today`, debtCounts.soon && `${debtCounts.soon} due soon`].filter(Boolean).join(" · ")}. Tap to view.
              </button>
              <button onClick={() => setDebtBannerHidden(true)} aria-label="Dismiss reminder" style={{ border: "none", background: "transparent", cursor: "pointer", color: T.slate, padding: 2, flexShrink: 0 }}><X size={14} /></button>
            </div>
          )}
          {loading ? (
            ...
            <>
              ...
              {tab === "debit-sales" && (
                <DebitSales debts={debitSales} currency={currency} canManage={activeStaff.role === "Admin"}
                  onSave={saveDebitSale} onSetPaid={setDebitPaid} onDelete={deleteDebitSale} />
              )}
              {tab === "refunds" && (
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} onClose={closeScan} allowedScans={allowedScans} lockMode={scanModeLocked}
          ...
          onRescan={rescan} cameraError={scanCameraError} setCameraError={setScanCameraError} onDetected={handleDetectedStable} onQuickAddNew={quickAddAndStockNewProduct}
...
/* ---------------------------------------------------------------
   Debit Sales — a plain notebook of items sold on credit. Nothing
   here touches stock, products, sales totals, reports, or inventory
   value: it only stores who owes what, and when they promised to pay.
   Reminders (nav badge + banner) are driven by due_date; see
   debtDueState() / DEBT_REMIND_DAYS_BEFORE near the top of the file.
------------------------------------------------------------------*/
function debtPill(d, nowStr) {
  const s = debtDueState(d, nowStr);
  switch (s.key) {
    case "paid": return { text: d.paid_on ? `PAID ${fmtDebtDate(d.paid_on)}` : "PAID", color: T.green };
    case "overdue": return { text: `OVERDUE ${s.days} DAY${s.days === 1 ? "" : "S"}`, color: T.stamp };
    case "today": return { text: "DUE TODAY", color: T.stamp };
    case "soon": return { text: s.days === 1 ? "DUE TOMORROW" : `DUE IN ${s.days} DAYS`, color: T.amber };
    case "upcoming": return { text: `DUE ${fmtDebtDate(d.due_date)}`, color: T.slate };
    default: return { text: "NO PAYMENT DATE", color: T.slateLight };
  }
}

function DebitSales({ debts, currency, canManage, onSave, onSetPaid, onDelete }) {
  const [filter, setFilter] = useState("unpaid"); // unpaid | paid | all
  const [search, setSearch] = useState("");
  const [editing, setEditing] = useState(null); // null = closed, "new" = blank form, or the debt being edited
  const nowStr = localDateStr();

  const unpaid = useMemo(() => debts.filter((d) => d.status !== "paid"), [debts]);
  const paid = useMemo(() => debts.filter((d) => d.status === "paid"), [debts]);
  const followUp = useMemo(() => unpaid.filter((d) => debtNeedsAttention(d, nowStr)).length, [unpaid, nowStr]);
  const totalOwed = useMemo(() => unpaid.reduce((s, d) => s + Number(d.amount || 0), 0), [unpaid]);

  const visible = useMemo(() => {
    const q = search.trim().toLowerCase();
    let rows = filter === "unpaid" ? unpaid : filter === "paid" ? paid : debts;
    if (q) {
      rows = rows.filter((d) =>
        (d.debtor_name || "").toLowerCase().includes(q) ||
        (d.item_name || "").toLowerCase().includes(q) ||
        (d.debtor_phone || "").replace(/\s+/g, "").includes(q.replace(/\s+/g, "")));
    }
    const sorted = [...rows];
    if (filter === "paid") {
      sorted.sort((a, b) => (b.paid_on || "").localeCompare(a.paid_on || ""));
    } else {
      // Unpaid first, soonest payment date first; records with no date sink
      // below the dated ones (newest first among themselves).
      sorted.sort((a, b) => {
        const ap = a.status === "paid", bp = b.status === "paid";
    ...
    return sorted;
  }, [debts, unpaid, paid, filter, search]);

  const btn = { padding: "7px 12px", borderRadius: 4, cursor: "pointer", fontSize: 11.5, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 5, ...body };

  return (
    <div>
      <PageHeader title="Debit Sales" sub="Items sold on credit. This page is a notebook only — it never changes your stock, sales, reports, or inventory value."
        actions={canManage ? [{ label: "Record Debit Sale", icon: Plus, onClick: () => setEditing("new"), solid: true }] : []} />

      <div className="tb-stat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 18 }}>
        <StatStub label="Unpaid" value={unpaid.length} accent={T.amber} />
        <StatStub label="Follow Up" value={followUp} accent={T.stamp} />
        <StatStub label="Total Owed" value={`${currency}${totalOwed.toLocaleString()}`} />
        <StatStub label="Paid" value={paid.length} accent={T.green} />
      </div>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", border: `1px solid ${T.ink}`, borderRadius: 4, overflow: "hidden" }}>
          {[["unpaid", `Unpaid (${unpaid.length})`], ["paid", `Paid (${paid.length})`], ["all", "All"]].map(([id, label]) => (
            <button key={id} onClick={() => setFilter(id)} style={{ padding: "7px 12px", border: "none", background: filter === id ? T.ink : "transparent", color: filter === id ? T.cream : T.ink, cursor: "pointer", fontSize: 12, fontWeight: 600, ...body }}>{label}</button>
          ))}
        </div>
        <div style={{ position: "relative", flex: 1, minWidth: 170 }}>
          <Search size={13} color={T.slateLight} style={{ position: "absolute", left: 10, top: 11 }} />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, item or phone" style={{ ...inputStyle, paddingLeft: 30 }} />
        </div>
      </div>

      {visible.length === 0 && (
        <Empty text={
          search.trim() ? "No debit records match that search." :
          filter === "paid" ? "Nothing has been marked as paid yet." :
          filter === "unpaid" ? (canManage ? "No unpaid debit sales. Tap Record Debit Sale when an item goes out on credit." : "No unpaid debit sales.") :
          "No debit sales recorded yet."
        } />
      )}

      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {visible.map((d) => {
          const pill = debtPill(d, nowStr);
          const isPaid = d.status === "paid";
          const phone = (d.debtor_phone || "").trim();
          return (
            <div key={d.id} style={{ background: T.cream, borderRadius: 4, padding: "12px 14px 12px 16px", borderLeft: `3px solid ${pill.color}`, opacity: isPaid ? 0.8 : 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>{d.debtor_name}</div>
                  <div style={{ fontSize: 12.5, marginTop: 2 }}>
                    {d.item_name} <span style={{ ...mono, color: T.slate }}>× {Number(d.qty).toLocaleString()}</span>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ ...mono, fontSize: 10.5, fontWeight: 700, letterSpacing: "0.04em", color: pill.color }}>{pill.text}</div>
                  {d.amount != null && <div style={{ ...mono, fontSize: 15, fontWeight: 700, marginTop: 3 }}>{currency}{Number(d.amount).toLocaleString()}</div>}
                </div>
              </div>

              <div style={{ fontSize: 11.5, color: T.slate, marginTop: 6 }}>
                Sold {fmtDebtDate(d.sale_date)}
                {d.due_date ? ` · Promised to pay ${fmtDebtDate(d.due_date)}` : ""}
                {d.recorded_by ? ` · Recorded by ${d.recorded_by}` : ""}
              </div>
              {d.note && <div style={{ fontSize: 11.5, color: T.slateLight, marginTop: 3, fontStyle: "italic" }}>"{d.note}"</div>}

              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
                {!isPaid && phone && (
                  <a href={`tel:${phone.replace(/[^\d+]/g, "")}`} style={{ ...btn, background: T.blue, color: "#fff", border: "none", textDecoration: "none" }}>
                    <Phone size={12} /> Call {phone}
                  </a>
                )}
                {!isPaid && !phone && canManage && (
                  <button onClick={() => setEditing(d)} style={{ ...btn, background: "transparent", border: `1px dashed ${T.slateLight}`, color: T.slate }}>
                    <Phone size={12} /> Add phone number
                  </button>
                )}
                {canManage && (
                  <>
                    <button onClick={() => onSetPaid(d, !isPaid)} style={isPaid
                      ? { ...btn, background: "transparent", border: `1px solid ${T.slate}`, color: T.slate }
                      : { ...btn, background: T.green, border: "none", color: T.cream }}>
                      <Check size={12} /> {isPaid ? "Mark unpaid" : "Mark paid"}
                    </button>
                    <button onClick={() => setEditing(d)} style={{ ...btn, background: "transparent", border: `1px solid ${T.slate}`, color: T.slate }}>
                      <Pencil size={12} /> Edit
                    </button>
                    <button onClick={() => onDelete(d)} style={{ ...btn, background: "transparent", border: `1px solid ${T.stamp}`, color: T.stamp }}>
      ...
      {editing && (
        <DebitSaleModal initial={editing === "new" ? null : editing} currency={currency} onSave={onSave} onClose={() => setEditing(null)} />
      )}
    </div>
  );
}

function DebitSaleModal({ initial, currency, onSave, onClose }) {
  const [form, setForm] = useState(() => initial ? {
    debtor_name: initial.debtor_name || "",
    debtor_phone: initial.debtor_phone || "",
    item_name: initial.item_name || "",
    qty: String(initial.qty ?? 1),
    amount: initial.amount == null ? "" : String(initial.amount),
    sale_date: initial.sale_date || localDateStr(),
    due_date: initial.due_date || "",
    note: initial.note || "",
  } : {
    debtor_name: "", debtor_phone: "", item_name: "", qty: "1", amount: "", sale_date: localDateStr(), due_date: "", note: "",
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const set = (k) => (e) => setForm({ ...form, [k]: e.target.value });
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (saving) return;
    if (!(Number(form.qty) > 0)) { setError("Quantity must be more than 0."); return; }
    if (form.due_date && form.due_date < form.sale_date) { setError("The payment date can't be earlier than the date sold."); return; }
    setError("");
    setSaving(true);
    const ok = await onSave(form, initial ? initial.id : null);
    setSaving(false);
    if (ok) onClose();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <form onSubmit={submit} style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "86vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>{initial ? "Edit Debit Sale" : "Record Debit Sale"}</h2>
          <button type="button" onClick={onClose} style={{ border: "none", background: "transparent", cursor: "pointer" }}><X size={18} /></button>
        </div>
        <div style={{ fontSize: 11.5, color: T.slate, marginBottom: 12, background: T.paper, borderRadius: 4, padding: "8px 10px" }}>
          This is a record only — saving it won't change your stock, sales, or inventory value.
        </div>

        <Field label="Debtor's name"><input required value={form.debtor_name} onChange={set("debtor_name")} style={inputStyle} autoFocus /></Field>
        <Field label="Phone number (so you can call)"><input type="tel" inputMode="tel" value={form.debtor_phone} onChange={set("debtor_phone")} style={inputStyle} placeholder="0801 234 5678" /></Field>
        <Field label="Item sold on debit"><input required value={form.item_name} onChange={set("item_name")} style={inputStyle} /></Field>
        <div style={{ display: "flex", gap: 10 }}>
          <div style={{ flex: 1 }}><Field label="Quantity"><input required type="number" min="0" step="any" value={form.qty} onChange={set("qty")} style={inputStyle} /></Field></div>
          <div style={{ flex: 1 }}><Field label={`Amount owed (${currency}) — optional`}><input type="number" min="0" step="any" value={form.amount} onChange={set("amount")} style={inputStyle} /></Field></div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <div style={{ flex: 1 }}><Field label="Date sold"><input required type="date" value={form.sale_date} onChange={set("sale_date")} style={inputStyle} /></Field></div>
          <div style={{ flex: 1 }}><Field label="Payment date"><input type="date" value={form.due_date} onChange={set("due_date")} style={inputStyle} /></Field></div>
        </div>
        <Field label="Note (optional)"><input value={form.note} onChange={set("note")} style={inputStyle} placeholder="e.g. will pay after salary" /></Field>

        {error && <div style={{ fontSize: 12, color: T.stamp, marginBottom: 10 }}>{error}</div>}
        <button type="submit" disabled={saving} style={{ width: "100%", marginTop: 6, padding: "10px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, fontWeight: 700, cursor: saving ? "default" : "pointer", opacity: saving ? 0.7 : 1, ...mono, fontSize: 12.5 }}>
          {saving ? "SAVING…" : "SAVE DEBIT RECORD"}
        </button>
      </form>
    </div>
  );
...
function ScanModal({
  ...
  cameraStage, missCode, unitError, lastResult, sessionCount, onRescan, cameraError, setCameraError, onDetected, onQuickAddNew,
  ...
  const [quickAddForm, setQuickAddForm] = useState({ name: "", category: "Medicine", purchase_price: "", selling_price: "", min_stock: "5", expiry: "" });
  // A new scan (different missCode, or none at all) should never reopen
  // showing a half-filled form for the previous code.
  useEffect(() => {
    setQuickAddOpen(false);
    setQuickAddForm({ name: "", category: "Medicine", purchase_price: "", selling_price: "", min_stock: "5", expiry: "" });
  }, [missCode]);
  const submitQuickAdd = async (e) => {
    e.preventDefault();
    if (!quickAddForm.name.trim() || quickAddSaving) return;
    setQuickAddSaving(true);
    await onQuickAddNew(missCode, quickAddForm);
          ...
          <div style={{ background: T.paper, borderRadius: 4, padding: "10px 12px", marginTop: 4 }}>
            {mode === "stock-in" ? (
              <>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setQuickAddOpen(true)} style={{ padding: "6px 10px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, cursor: "pointer", fontSize: 11.5, fontWeight: 600 }}>It's a brand-new item</button>
                ...
                {quickAddOpen && (
                  <form onSubmit={submitQuickAdd} style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.paperDim}` }}>
                    <div style={{ fontSize: 11.5, color: T.slate, marginBottom: 8 }}>
                      Registering <strong style={{ ...mono }}>{missCode}</strong> as a new item \u2014 it'll be stocked in with this exact code the moment you save, no relabeling needed.
                    </div>
                    <input required autoFocus placeholder="Item name" value={quickAddForm.name} onChange={(e) => setQuickAddForm((f) => ({ ...f, name: e.target.value }))} style={{ width: "100%", padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, marginBottom: 6, boxSizing: "border-box" }} />
                    <select value={quickAddForm.category} onChange={(e) => setQuickAddForm((f) => ({ ...f, category: e.target.value }))} style={{ width: "100%", padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, marginBottom: 6, boxSizing: "border-box" }}>
                      {CATEGORIES.map((c) => <option key={c}>{c}</option>)}
                    </select>
                    <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                      <input required type="number" step="0.01" placeholder="Purchase price" value={quickAddForm.purchase_price} onChange={(e) => setQuickAddForm((f) => ({ ...f, purchase_price: e.target.value }))} style={{ flex: 1, padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, boxSizing: "border-box" }} />
                      <input required type="number" step="0.01" placeholder="Selling price" value={quickAddForm.selling_price} onChange={(e) => setQuickAddForm((f) => ({ ...f, selling_price: e.target.value }))} style={{ flex: 1, padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, boxSizing: "border-box" }} />
                    </div>
                    <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                      <input type="number" placeholder="Min stock (alert level)" value={quickAddForm.min_stock} onChange={(e) => setQuickAddForm((f) => ({ ...f, min_stock: e.target.value }))} style={{ flex: 1, padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, boxSizing: "border-box" }} />
                      <input type="date" title="Expiry (optional)" value={quickAddForm.expiry} onChange={(e) => setQuickAddForm((f) => ({ ...f, expiry: e.target.value }))} style={{ flex: 1, padding: "8px 9px", borderRadius: 4, border: `1px solid ${T.paperDim}`, fontSize: 13, boxSizing: "border-box" }} />
                    </div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button type="submit" disabled={quickAddSaving} style={{ padding: "7px 12px", borderRadius: 4, border: "none", background: T.green, color: "#fff", cursor: quickAddSaving ? "default" : "pointer", fontSize: 11.5, fontWeight: 700, opacity: quickAddSaving ? 0.7 : 1 }}>
                        {quickAddSaving ? "SAVING\u2026" : "SAVE & ADD TO BATCH"}
                      </button>
                      <button type="button" onClick={() => setQuickAddOpen(false)} style={{ padding: "7px 12px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", cursor: "pointer", fontSize: 11.5 }}>Back</button>
                    </div>
                  </form>
                )}
              </>
