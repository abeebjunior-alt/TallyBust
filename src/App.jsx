      ...
      <div className={`tb-sidebar${mobileNavOpen ? " nav-open" : ""}`} style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div className="tb-nav-list" style={{ display: "contents" }}>
          {NAV.map((n) => {
            ...
            const showRefundBadge = n.id === "dashboard" && activeStaff.role === "Admin" && pendingRefunds.length > 0;
            return (
            ...
            <>
              {tab === "dashboard" && (
                <Dashboard settings={settings} itemsSoldToday={itemsSoldToday} stockAddedToday={stockAddedToday} todaysSales={todaysSales}
                  ...
                  isAdminRole={activeStaff.role === "Admin"} pendingRefunds={pendingRefunds} onResolveRefund={resolveRefund} />
              )}
              {tab === "inventory" && (
...
/* ---------------------------------------------------------------
   Dashboard
------------------------------------------------------------------*/
function Dashboard({ settings, itemsSoldToday, stockAddedToday, todaysSales, inventoryValue, totalStockUnits, products, lowStock, outOfStock, expiringSoon, bestSellers, weeklyChart, currency, onScan, allowedScans, onSelectProduct, isAdminRole, pendingRefunds, onResolveRefund }) {
  ...
  return (
    <div>
      ...
      {isAdminRole && (
        <div style={{ background: T.cream, borderRadius: 4, padding: "14px 18px", marginBottom: 18, border: `1px solid ${pendingRefunds.length ? T.amber : T.paperDim}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: pendingRefunds.length ? 10 : 0 }}>
            <RotateCcw size={15} color={pendingRefunds.length ? T.amber : T.slate} />
            <div style={{ fontWeight: 700, fontSize: 13.5 }}>Pending Refunds{pendingRefunds.length > 0 ? ` (${pendingRefunds.length})` : ""}</div>
          </div>
          {pendingRefunds.length === 0 ? (
            <div style={{ fontSize: 12, color: T.slate }}>No refund requests waiting for review.</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {pendingRefunds.map((r) => (
                <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, background: T.paper, borderRadius: 4, padding: "9px 12px" }}>
