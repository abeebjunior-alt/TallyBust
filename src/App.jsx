...
import {
  ...
  CheckSquare, Square, Boxes, Minus, ClipboardCheck,
} from "lucide-react";
...
const RESPONSIVE_CSS = `
  * { box-sizing: border-box; }
  ...
  @media (max-width: 760px) {
    ...
    .tb-logo { padding: 6px 12px !important; flex-shrink: 0; cursor: pointer; background: rgba(255,255,255,0.08); border-radius: 20px; }
    ...
    .tb-scan-quick-btn { padding: 8px 16px !important; border-radius: 20px !important; font-size: 12px !important; }
...
function TallyBust({ userId, userEmail, initialActiveStaff }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [{ data: prod, error: e1 }, { data: hist, error: e2 }, { data: settingsRow, error: e3 }, { data: configRow }, { data: staffRows }, { data: refundRows }, { data: unitRows }] = await Promise.all([
  ...
  const inventoryValue = useMemo(() => {
    ...
    for (const p of products) {
      ...
      const counted = units.filter((u) => u.status === "in_stock" || u.status === "opened");
      total += counted.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0);
      const gap = Math.max(0, p.qty - counted.length);
  ...
  const openScan = (mode) => {
    ...
    setScanMode(mode);
    setScanMissCode(""); setScanCameraError(""); setScanUnitError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch(""); setScanWriteOffPending(null);
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanMode(newMode);
    setScanMissCode(""); setScanUnitError(""); setScanCameraError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch(""); setScanWriteOffPending(null);
    setScanCameraStage("scanning");
  };
  ...
  // after that it behaves exactly like any of our own codes (one-time
  // use, scan it out to sell, scan it to refund, etc.).
  const linkExternalCode = async (rawCode, product) => {
    const code = rawCode.trim();
    const nowIso = new Date().toISOString();
    const { error: e1 } = await supabase.from("product_units").insert({ user_id: userId, product_id: product.id, code, status: "in_stock", stocked_in_at: nowIso, purchase_price: product.purchase_price });
    if (e1) { fireToast(`Could not link code: ${e1.message}`); return; }
    const { error: e2 } = await supabase.from("products").update({ qty: product.qty + 1 }).eq("id", product.id);
    const { error: e3 } = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "stock-in", qty: 1, unit_price: product.purchase_price, unit_code: code, staff: activeStaff.name, occurred_on: todayStr });
    if (e2 || e3) { fireToast(`Save failed: ${(e2 || e3).message}`); }
    setScanLastResult({ ok: true, text: `Stocked in \u2014 ${product.name}`, code });
  ...
  // Fires the instant every single scan goes through: find the exact
  // physical item behind this code, check it's in the right state for
  // what we're doing, apply the one-unit stock movement, then go
  // straight back to scanning for the next item — no quantity, no
  // confirm tap, no manual product picking (Write Off is the one
  // exception — it needs a reason, see finalizeWriteOff above).
  const handleDetectedCode = async (rawCode) => {
    ...
    if (scanMode === "stock-in") {
      ...
      const { error: e1 } = await supabase.from("product_units").update({ status: "in_stock", stocked_in_at: new Date().toISOString() }).eq("id", unit.id);
      const { error: e2 } = await supabase.from("products").update({ qty: product.qty + 1 }).eq("id", product.id);
      const { error: e3 } = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "stock-in", qty: 1, unit_price: unit.purchase_price ?? product.purchase_price, unit_code: unit.code, staff: activeStaff.name, occurred_on: todayStr });
      if (e1 || e2 || e3) { fireToast(`Save failed: ${(e1 || e2 || e3).message}`); setTimeout(rescan, 1000); return; }
      setScanLastResult({ ok: true, text: `Stocked in \u2014 ${product.name}`, code: unit.code });
      ...
      const { error: e1 } = await supabase.from("product_units").update({ status: "sold", sold_at: new Date().toISOString() }).eq("id", unit.id);
      const { error: e2 } = await supabase.from("products").update({ qty: Math.max(0, product.qty - 1) }).eq("id", product.id);
      const { error: e3 } = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "sale", qty: 1, unit_price: product.selling_price, unit_code: unit.code, staff: activeStaff.name, occurred_on: todayStr });
      if (e1 || e2 || e3) { fireToast(`Save failed: ${(e1 || e2 || e3).message}`); setTimeout(rescan, 1000); return; }
      setScanLastResult({ ok: true, text: `Sold \u2014 ${product.name}`, code: unit.code });
    } else if (scanMode === "refund") {
      if (unit.status !== "sold") {
  ...
  // save) — this writes off the shortfall so the numbers match reality,
  // and records why. Counting MORE just gets flagged, since we can't know
  // which physical items to invent codes for — that's what "tag & print"
  // on the Labels page is for.
  ...
  return (
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} onClose={() => setScanOpen(false)} allowedScans={allowedScans}
...
function StockHealth({ products, unitCounts, history, currency, canWriteOff, onScanWriteOff, onSubmitReconciliation }) {
...
function Reports({ products, history, currency, inventoryValue, onExport, refunds, productUnits }) {
  ...
  const valueOf = (p) => {
    const units = (productUnits || []).filter((u) => u.product_id === p.id && (u.status === "in_stock" || u.status === "opened"));
    const counted = units.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0);
    const gap = Math.max(0, p.qty - units.length);
    return counted + gap * Number(p.purchase_price);
  };
      ...
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ ...mono, fontSize: 10.5, color: T.slate }}>
            {modeLabel} — scan one item at a time, no typing needed.
          </div>
          {sessionCount > 0 && <div style={{ ...mono, fontSize: 11, fontWeight: 700, color: T.ink }}>{sessionCount} scanned</div>}
        </div>
