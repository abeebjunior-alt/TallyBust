...
const ROLE_TABS = {
  Admin: ["dashboard", "inventory", "labels", "sales", "unit-sales", "refunds", "reports", "staff", "settings"],
...
function TallyBust({ userId, userEmail, initialActiveStaff }) {
  ...
  const openScan = (mode) => {
    ...
    setScanMode(mode);
    setScanMissCode(""); setScanCameraError(""); setScanUnitError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch("");
  ...
  const rescan = () => {
    setScanMissCode(""); setScanUnitError(""); setScanCameraError(""); setScanLinkSearch("");
    setScanCameraStage("scanning");
  };
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanMode(newMode);
    setScanMissCode(""); setScanUnitError(""); setScanCameraError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch("");
    setScanCameraStage("scanning");
  };
  ...
  const linkExternalCode = async (rawCode, product) => {
    ...
    const { error: e1 } = await supabase.from("product_units").insert({ user_id: userId, product_id: product.id, code, status: "in_stock", stocked_in_at: nowIso });
    ...
    const { error: e3 } = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "stock-in", qty: 1, unit_price: product.purchase_price, staff: activeStaff.name, occurred_on: todayStr });
  ...
  // confirm tap, no manual product picking.
  const handleDetectedCode = async (rawCode) => {
    ...
    if (scanMode === "stock-in") {
      if (unit.status !== "unstocked") {
        setScanUnitError(unit.status === "in_stock" ? `${product.name} (${unit.code}) is already stocked in.` : `${product.name} (${unit.code}) was already sold \u2014 it can't be stocked in again.`);
        setTimeout(rescan, 1400);
        return;
      }
      ...
      if (unit.status !== "in_stock") {
        setScanUnitError(unit.status === "sold" ? `${product.name} (${unit.code}) was already sold.` : `${product.name} (${unit.code}) hasn't been stocked in yet.`);
        setTimeout(rescan, 1400);
        return;
      }
      ...
      const { error: e3 } = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "sale", qty: 1, unit_price: product.selling_price, staff: activeStaff.name, occurred_on: todayStr });
      if (e1 || e2 || e3) { fireToast(`Save failed: ${(e1 || e2 || e3).message}`); setTimeout(rescan, 1000); return; }
      ...
      const { error } = await supabase.from("refunds").insert({ user_id: userId, product_id: product.id, product_name: product.name, qty: 1, unit_code: unit.code, staff: activeStaff.name });
  ...
  const approveRefund = async (refund) => {
    ...
    const unit = refund.unit_code ? productUnits.find((u) => u.code === refund.unit_code) : null;
    ...
    let e4 = null;
    if (unit) {
      const res = await supabase.from("product_units").update({ status: "in_stock", stocked_in_at: new Date().toISOString() }).eq("id", unit.id);
      e4 = res.error;
    }
  ...
  // that were already in stock before this system existed.
...
function Refunds({ refunds, canScanRefund, onScan, canManageRefunds, onApproveRefund, onRejectRefund }) {
  ...
  return (
    <div>
      <PageHeader title="Refunds" sub="Items returned by a customer — scanned in, then confirmed by an Admin before they're added back to stock."
        ...
        <Panel title="Refund History">
          ...
          {resolved.map((r) => (
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${T.paperDim}`, flexWrap: "wrap", gap: 8 }}>
              ...
              <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: r.status === "approved" ? T.green : T.stamp, textTransform: "uppercase" }}>{r.status}</span>
            </div>
          ))}
        </Panel>
      </div>
...
function ScanModal({
  ...
  const modeLabel = mode === "stock-in" ? "Stock In" : mode === "sale" ? "Stock Out" : "Refund";
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(20,20,15,0.55)" }} />
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        {cameraStage === "off" && !cameraError && !missCode && (
