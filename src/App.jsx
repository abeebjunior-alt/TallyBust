...
import {
  ...
  Search, AlertTriangle, Download, X, Check, ChevronRight, ChevronDown, Shuffle, ArrowUpCircle,
...
const RESPONSIVE_CSS = `
  * { box-sizing: border-box; }
  ...
  @media (max-width: 760px) {
    ...
    .tb-switchuser-btn { padding: 8px 14px !important; border-radius: 20px !important; border: 1px solid ${T.blueSoft} !important; background: transparent !important; color: ${T.blueSoft} !important; font-size: 12px !important; margin-top: 0 !important; }
...
function TallyBust({ userId, userEmail, initialActiveStaff, onLock }) {
  ...
  const [scanNotSold, setScanNotSold] = useState(null);
  ...
  // A product can only be refunded once it has actually been sold (Stock
  // Out) at some point — never right after it's been registered/stocked in.
  const soldProductIds = useMemo(() => new Set(history.filter((h) => h.type === "sale").map((h) => h.product_id)), [history]);
  ...
  const openScan = (mode) => {
    ...
    setScanMissCode(""); setScanCameraError(""); setScanOutOfStock(null); setScanNotSold(null);
    // Stock In / Stock Out / Refund are all camera-only — no manual product entry.
  ...
  const handleDetectedCode = (rawCode) => {
    ...
    if (match) {
      if (scanMode === "sale" && match.qty <= 0) {
        ...
        setScanNotSold(null);
        setScanProductId("");
      } else if (scanMode === "refund" && !soldProductIds.has(match.id)) {
        // Never let a fresh, just-registered item be refunded — it has to
        // have gone out (Stock Out) to a customer at least once first.
        setScanMissCode("");
        setScanOutOfStock(null);
        setScanNotSold(match);
        setScanProductId("");
      } else {
        setScanMissCode("");
        setScanOutOfStock(null);
  ...
  const rescan = () => {
    setScanOutOfStock(null); setScanNotSold(null); setScanMissCode(""); setScanCameraError("");
    setScanCameraStage("scanning");
  };
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanMissCode(""); setScanOutOfStock(null); setScanNotSold(null); setScanCameraError("");
    setScanCameraStage("scanning");
  };
  ...
  const confirmScan = async () => {
    ...
    // Defensive guard: only ever allow a refund for something that has
    // actually been sold before (the scan flow already blocks this earlier).
    if (scanMode === "refund") {
      if (!soldProductIds.has(scanProduct.id)) { fireToast(`${scanProduct.name} hasn't been sold yet — it can't be refunded.`); return; }
    ...
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className="tb-sidebar" style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div className="tb-sidebar-foot" style={{ marginTop: "auto", paddingTop: 18, borderTop: `1px solid ${T.inkSoft}` }}>
          ...
          {onLock && (
            <button onClick={onLock} className="tb-switchuser-btn" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 6, padding: "7px", borderRadius: 4, border: "none", background: "transparent", color: T.slateLight, cursor: "pointer", fontSize: 11 }}>
              <Lock size={12} /> <span className="tb-switchuser-label">Switch user</span>
            </button>
          )}
          <button onClick={signOut} className="tb-signout-btn" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 4, padding: "7px", borderRadius: 4, border: "none", background: "transparent", color: T.slateLight, cursor: "pointer", fontSize: 11 }}>
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} candidates={scanCandidates} search={scanSearch} setSearch={setScanSearch}
          ...
          missCode={scanMissCode} setMissCode={setScanMissCode} outOfStock={scanOutOfStock} notSold={scanNotSold} onRescan={rescan}
...
function ScanModal({
  ...
  cameraStage, setCameraStage, missCode, setMissCode, outOfStock, notSold, onRescan, cameraError, setCameraError, onDetected, onAddNew,
}) {
  ...
  const manualAllowed = false; // every scan type (Stock In / Stock Out / Refund) is barcode-only — no manual product entry
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(20,20,15,0.55)" }} />
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        {!manualAllowed && (
          <div style={{ ...mono, fontSize: 10.5, color: T.slate, marginBottom: 10 }}>
            {mode === "stock-in" ? "Stock In" : mode === "sale" ? "Stock Out" : "Refund"} is barcode-only — scan the product's sticker to continue.
          </div>
        )}
        {outOfStock ? (
          ...
          <div style={{ textAlign: "center", padding: "18px 10px" }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{notSold.name} hasn't been sold yet</div>
            <div style={{ fontSize: 12.5, color: T.slate, marginBottom: 16 }}>Only items that were stocked in and then sold (Stock Out) to a customer can be refunded — not right after registering it.</div>
            <button onClick={onRescan} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 4, border: "none", background: T.ink, color: T.cream, cursor: "pointer", fontSize: 12.5, fontWeight: 600, ...mono }}>
              <Camera size={14} /> SCAN ANOTHER
            </button>
          </div>
