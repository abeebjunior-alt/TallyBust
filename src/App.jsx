...
function SubscriptionExpired({ config, businessName, onSignOut }) {
  return (
    <div style={{ ...body, minHeight: "100vh", background: T.ink, color: T.cream, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ maxWidth: 420, width: "100%", background: T.cream, color: T.ink, borderRadius: 8, padding: "28px 26px", textAlign: "center" }}>
        ...
        <button onClick={onSignOut} style={{ padding: "10px 18px", borderRadius: 4, border: `1px solid ${T.paperDim}`, background: "transparent", color: T.slate, cursor: "pointer", fontSize: 12.5, fontWeight: 600, ...mono }}>
          Sign out
        </button>
      </div>
    </div>
...
function TallyBust({ userId, userEmail }) {
  ...
  const [scanCameraStage, setScanCameraStage] = useState("off"); // off | selfie | scanning
  const [scanSelfie, setScanSelfie] = useState(null);
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const [{ data: prod, error: e1 }, { data: hist, error: e2 }, { data: settingsRow, error: e3 }, { data: configRow }] = await Promise.all([
  ...
  const openScan = (mode) => {
    ...
    setScanSelfie(null); setScanMissCode(""); setScanCameraError(""); setScanOutOfStock(null);
    // Stock In / Stock Out are camera-only — jump straight to the selfie step.
    // Stock Count still allows manual search/selection.
    setScanCameraStage(mode === "count" ? "off" : "selfie");
    setScanOpen(true);
  };
  ...
  const rescan = () => {
    setScanOutOfStock(null); setScanMissCode(""); setScanCameraError(""); setScanSelfie(null);
    setScanCameraStage("selfie");
  };

  const switchScanMode = (newMode) => {
    setScanMode(newMode);
    setScanProductId(""); setScanQty(""); setScanSearch("");
    setScanMissCode(""); setScanOutOfStock(null); setScanSelfie(null); setScanCameraError("");
    setScanCameraStage(newMode === "count" ? "off" : "selfie");
  };
  ...
  const confirmScan = async () => {
    ...
    const { error: e2 } = await supabase.from("stock_history").insert({
      ...
      captured_photo: scanSelfie || null,
    });
  ...
  if (!isAdmin && !subState.active) {
    return <SubscriptionExpired T={T} mono={mono} body={body} config={appConfig} businessName={settings.business_name} onSignOut={signOut} />;
  }
    ...
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className="tb-sidebar" style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div className="tb-sidebar-foot" style={{ marginTop: "auto", paddingTop: 18, borderTop: `1px solid ${T.inkSoft}` }}>
          <button onClick={() => openScan("stock-in")} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 10px", borderRadius: 4, border: "none", background: T.blue, color: "#fff", cursor: "pointer", fontWeight: 700, ...mono, fontSize: 12.5 }}>
            <ScanLine size={16} /> SCAN
          </button>
          <div className="tb-user-email" style={{ ...mono, fontSize: 9.5, color: T.slateLight, marginTop: 10, textAlign: "center", wordBreak: "break-all" }}>{userEmail}</div>
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} candidates={scanCandidates} search={scanSearch} setSearch={setScanSearch}
          ...
          cameraStage={scanCameraStage} setCameraStage={setScanCameraStage} selfie={scanSelfie} setSelfie={setScanSelfie}
...
/* ---------------------------------------------------------------
   Dashboard
------------------------------------------------------------------*/
function Dashboard({ settings, itemsSoldToday, stockAddedToday, todaysSales, inventoryValue, totalStockUnits, products, lowStock, outOfStock, expiringSoon, bestSellers, weeklyChart, currency, onScan, onSelectProduct }) {
...
function Sales({ history, currency }) {
  ...
  return (
    <div>
      ...
      <div className="tb-table-scroll" style={{ background: T.cream, borderRadius: 4, overflow: "hidden" }}>
       <div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr 0.6fr 0.9fr 0.9fr 0.5fr 0.9fr", padding: "10px 16px", ...mono, fontSize: 10.5, letterSpacing: "0.06em", textTransform: "uppercase", color: T.slate, borderBottom: `1px solid ${T.paperDim}` }}>
          <span>Date</span><span>Product</span><span>Qty</span><span>Unit Price</span><span>Total</span><span>Photo</span><span>Staff</span>
        </div>
        {sales.slice(0, 40).map((h) => (
          <div key={h.id} style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr 0.6fr 0.9fr 0.9fr 0.5fr 0.9fr", padding: "9px 16px", alignItems: "center", fontSize: 12.5, borderBottom: `1px solid ${T.paperDim}` }}>
            ...
            <span>
              {h.captured_photo ? (
                <img src={h.captured_photo} alt="Sold by" onClick={() => setZoomPhoto(h.captured_photo)} style={{ width: 24, height: 24, borderRadius: 4, objectFit: "cover", cursor: "pointer" }} />
              ) : (
                <span style={{ fontSize: 10.5, color: T.slateLight }}>—</span>
              )}
            </span>
      ...
      {zoomPhoto && (
        <div onClick={() => setZoomPhoto(null)} style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <img src={zoomPhoto} alt="Sold by" style={{ maxWidth: "90vw", maxHeight: "90vh", borderRadius: 8 }} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", justifyContent: "flex-end" }}>
      ...
      <div style={{ position: "relative", width: 380, maxWidth: "100vw", background: T.cream, height: "100%", padding: "22px 20px", overflow: "auto" }}>
        ...
        {withBalance.map((h) => (
          <div key={h.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${T.paperDim}`, fontSize: 12.5 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {h.captured_photo && (
                <img src={h.captured_photo} alt="Captured at scan" onClick={() => setZoomPhoto(h.captured_photo)} style={{ width: 30, height: 30, borderRadius: 4, objectFit: "cover", cursor: "pointer", flexShrink: 0 }} />
              )}
      ...
      {zoomPhoto && (
        <div onClick={() => setZoomPhoto(null)} style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <img src={zoomPhoto} alt="Captured at scan" style={{ maxWidth: "90vw", maxHeight: "90vh", borderRadius: 8 }} />
        </div>
      )}
    </div>
  );
...
function ScanModal({
  ...
  cameraStage, setCameraStage, selfie, setSelfie, missCode, setMissCode, outOfStock, onRescan, cameraError, setCameraError, onDetected, onAddNew,
}) {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        {outOfStock ? (
          ...
          <SelfieCapture
            T={T} mono={mono} body={body}
            onCapture={(img) => { setSelfie(img); setCameraStage("scanning"); }}
            onCancel={() => { manualAllowed ? setCameraStage("off") : onClose(); }}
          />
        ) : !selected ? (
          <>
            {manualAllowed ? (
              <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                <button
                  onClick={() => { setMissCode(""); setCameraError(""); setSelfie(null); setCameraStage(cameraOn ? "off" : "selfie"); }}
            ...
            {cameraStage === "scanning" && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, ...mono, fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: T.slate }}>
                  {selfie && <img src={selfie} alt="Captured" style={{ width: 26, height: 26, borderRadius: 4, objectFit: "cover" }} />}
                  Step 2 of 2 — Scan the barcode
                </div>
