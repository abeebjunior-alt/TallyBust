...
import {
  ...
  CheckSquare, Square, Boxes, Minus, ClipboardCheck, LifeBuoy,
} from "lucide-react";
import { supabase, supabaseStaffAuth } from "./supabaseClient";
...
const ROLE_TABS = {
  Admin: ["dashboard", "inventory", "labels", "sales", "unit-sales", "refunds", "stock-health", "reports", "staff", "settings"],
  Manager: ["dashboard", "inventory", "labels", "sales", "reports"],
  Cashier: ["dashboard", "sales", "unit-sales", "refunds"],
...
const ROLE_SCAN_ACTIONS = {
  ...
  Manager: [],
  Cashier: ["sale", "refund"],
  Storekeeper: ["stock-in", "refund", "write-off"],
};
...
function TallyBust({ userId, userEmail, initialActiveStaff }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [{ data: prod, error: e1 }, { data: hist, error: e2 }, { data: settingsRow, error: e3 }, { data: configRow }, { data: staffRows }, { data: refundRows }, { data: writeOffRows }, { data: unitRows }] = await Promise.all([
      ...
      supabase.from("write_offs").select("*").order("requested_at", { ascending: false }).limit(300),
      supabase.from("product_units").select("*").order("created_at", { ascending: false }).limit(3000),
    ]);
  ...
  const todaysSalesGross = useMemo(() => todayHistory.filter((h) => h.type === "sale" || h.type === "unit-sale").reduce((s, h) => s + h.qty * Number(h.unit_price), 0), [todayHistory]);
  // Refunds reduce reported revenue in the period they're actually
  // approved in (not retroactively restated against the original sale
  // day) — this applies whether the item got restocked or written off,
  // since either way the money genuinely went back to the customer.
  const todaysRefundValue = useMemo(() => refunds.filter((r) => r.status === "approved" && r.reviewed_at && r.reviewed_at.slice(0, 10) === todayStr).reduce((s, r) => s + Number(r.sale_price || 0), 0), [refunds]);
  const todaysSales = todaysSalesGross - todaysRefundValue;
  ...
  // Sums what was actually paid for each unit still on hand (in_stock or
  // opened-for-loose-sale) instead of just multiplying today's price by
  // however many are left — so a price change doesn't retroactively
  // reprice stock bought before it. Any stock with no per-unit record yet
  // (added before this tracking existed) falls back to today's price.
  // Sums what was actually paid for each unit still on hand instead of
  // just multiplying today's price by however many are left. A pack that
  // gets opened for loose-unit sales is deliberately NOT counted at its
  // full price here — its value now lives in the loose_units pool below,
  // shrinking as each loose unit actually gets sold (or growing back if
  // one gets refunded), instead of staying stuck at "one whole pack"
  // forever after the first tablet is sold out of it.
  const inventoryValue = useMemo(() => {
    let total = 0;
    for (const p of products) {
      const units = unitsByProduct.get(p.id) || [];
      // A unit awaiting write-off confirmation is still physically on the
      // shelf and still counts toward qty, so it still counts here too —
      // the value only actually leaves once an Admin approves the write
      // off (see approveWriteOff).
      const sealed = units.filter((u) => u.status === "in_stock" || u.status === "pending_write_off");
      total += sealed.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0);
      const gap = Math.max(0, p.qty - sealed.length); // legacy stock with no per-unit record yet
      total += gap * Number(p.purchase_price);

      if (p.unit_sale_enabled && p.loose_units > 0) {
        const perPack = Number(p.unit_qty_per_pack) || 1;
        const openedUnits = units.filter((u) => u.status === "opened");
        const avgCost = openedUnits.length > 0
          ? openedUnits.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0) / openedUnits.length
          : Number(p.purchase_price);
        total += (p.loose_units / perPack) * avgCost;
      }
    }
    return total;
  ...
  const totalProductsAvailable = useMemo(() => products.filter((p) => p.qty > 0 || (p.unit_sale_enabled && (p.loose_units || 0) > 0)).length, [products]);
  ...
  const unitCounts = useMemo(() => {
    ...
    for (const [pid, units] of unitsByProduct) {
      map.set(pid, {
        ...
        opened: units.filter((u) => u.status === "opened").length,
  ...
  const addStaffMember = async (form) => {
    ...
    if (!settings.business_code) { fireToast("Still setting up this business's login code \u2014 try again in a moment."); return; }

    // Give this staff member their own, completely independent Supabase
    // Auth account (built from Business Code + Username + PIN) using the
    // isolated helper client, so this never disturbs the Owner's own
    // signed-in session on this device.
    const email = staffAuthEmail(form.name, settings.business_code);
    const password = staffAuthPassword(form.pin);
    const { data: signUpData, error: signUpErr } = await supabaseStaffAuth.auth.signUp({ email, password });
    if (signUpErr) { fireToast(`Could not create their login: ${signUpErr.message}`); return; }
    const authUserId = signUpData.user ? signUpData.user.id : null;

    const { error } = await supabase.from("staff").insert({ user_id: userId, auth_user_id: authUserId, ...form });
  ...
  // gives them a real account now, using their existing username/PIN.
  const setupStaffLogin = async (member) => {
    if (!settings.business_code) { fireToast("Still setting up this business's login code \u2014 try again in a moment."); return; }
    const email = staffAuthEmail(member.name, settings.business_code);
    const password = staffAuthPassword(member.pin);
    const { data: signUpData, error: signUpErr } = await supabaseStaffAuth.auth.signUp({ email, password });
    if (signUpErr) { fireToast(`Could not set up their login: ${signUpErr.message}`); return; }
    const authUserId = signUpData.user ? signUpData.user.id : null;
    const { error } = await supabase.from("staff").update({ auth_user_id: authUserId }).eq("id", member.id);
    if (error) { fireToast(`Could not save their login: ${error.message}`); return; }
  ...
  const openScan = (mode) => {
    ...
    setScanMode(mode);
    setScanMissCode(""); setScanCameraError(""); setScanUnitError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch(""); setScanWriteOffPending(null); setScanPile([]);
  ...
  // the next item.
  const rescan = () => {
    setScanMissCode(""); setScanUnitError(""); setScanCameraError(""); setScanLinkSearch(""); setScanWriteOffPending(null);
    setScanCameraStage("scanning");
  };
  ...
  const switchScanMode = (newMode) => {
    ...
    setScanMode(newMode);
    setScanMissCode(""); setScanUnitError(""); setScanCameraError(""); setScanLastResult(null); setScanSessionCount(0); setScanLinkSearch(""); setScanWriteOffPending(null); setScanPile([]);
    setScanCameraStage("scanning");
  };
  ...
  // Scanning Write Off no longer removes the item from the books on the
  // spot — it locks the exact unit (so it can't be sold, refunded, or
  // written off again) and files a pending request. Nothing is deducted
  // from qty or Inventory Value, and nothing hits the Write-Off Log,
  // until an Admin reviews it below (see approveWriteOff).
  const finalizeWriteOff = async (reason) => {
    ...
    const { error: e2 } = await supabase.from("write_offs").insert({ user_id: userId, product_id: product.id, product_name: product.name, qty: 1, unit_code: unit.code, purchase_price: unit.purchase_price ?? product.purchase_price, reason, staff: activeStaff.name });
    if (e1 || e2) { fireToast(`Save failed: ${(e1 || e2).message}`); setScanWriteOffPending(null); setTimeout(rescan, 1000); return; }
    setScanLastResult({ ok: true, text: `Write off requested (${reason}) \u2014 ${product.name}`, code: unit.code });
  ...
  // Admin reviews a pending write-off. Approving is the moment the loss
  ...
  const handleDetectedCode = async (rawCode) => {
    ...
    if (scanMode === "sale") {
      if (unit.status !== "in_stock") {
        setScanUnitError(unit.status === "sold" ? `${product.name} (${unit.code}) was already sold.` : unit.status === "opened" ? `${product.name} (${unit.code}) was opened for loose-unit sales \u2014 it's no longer a whole pack.` : `${product.name} (${unit.code}) isn't available to sell (currently ${unit.status.replace("_", " ")}).`);
        setTimeout(rescan, 1400);
        return;
      }
      if (scanPile.some((it) => it.code === unit.code)) {
    ...
    if (scanMode === "refund") {
      if (unit.status !== "sold") {
        setScanUnitError(`${product.name} (${unit.code}) hasn't been sold \u2014 only a sold item can be refunded.`);
        setTimeout(rescan, 1400);
        return;
      }
      ...
      const { error } = await supabase.from("refunds").insert({ user_id: userId, product_id: product.id, product_name: product.name, qty: 1, unit_code: unit.code, sale_price: salePrice, staff: activeStaff.name });
      if (error) { fireToast(`Save failed: ${error.message}`); setTimeout(rescan, 1000); return; }
      setScanLastResult({ ok: true, text: `Refund logged \u2014 ${product.name}`, code: unit.code });
  ...
  const approveRefund = async (refund, disposition) => {
    ...
    if (refund.unit_code) {
      ...
      // A quantity of loose units (no single code to flip).
      if (disposition === "restock") {
        const r2 = await supabase.from("products").update({ loose_units: (product.loose_units || 0) + refund.qty }).eq("id", product.id);
        const r3 = await supabase.from("stock_history").insert({ user_id: userId, product_id: product.id, product_name: product.name, type: "unit-refund", qty: refund.qty, unit_price: product.unit_price, staff: activeStaff.name, occurred_on: todayStr });
        err = r2.error || r3.error;
      }
      // write_off disposition for loose units: nothing physical to move —
      // it's already been consumed, so only the revenue side is netted.
    }
  ...
  // A refund request for a quantity of loose units (tablets, etc. — no
  // single scannable code exists for these). Goes through the same
  // Admin review/disposition flow as a scanned packed-item refund.
  const requestLooseRefund = async (productId, qty) => {
    const product = allProducts.find((p) => p.id === productId);
    if (!product || qty <= 0) return;
    const { error } = await supabase.from("refunds").insert({
      user_id: userId, product_id: product.id, product_name: product.name, qty,
      unit_code: null, sale_price: Number(product.unit_price || 0) * qty, staff: activeStaff.name,
    });
    if (error) { fireToast(`Could not log return: ${error.message}`); return; }
    fireToast(`Return logged for ${qty} ${product.unit_name || "unit"}${qty === 1 ? "" : "s"} of ${product.name} \u2014 awaiting Admin confirmation`);
    loadAll();
  };
  ...
  // save) — this files a write-off request for the shortfall, same as
  // scanning Write Off does, so it still needs Admin confirmation below
  // before qty or Inventory Value actually change. Counting MORE just
  // gets flagged, since we can't know which physical items to invent
  // codes for — that's what "tag & print" on the Labels page is for.
  const submitReconciliation = async (counts) => {
    let shortfalls = 0, unitsFlagged = 0, overages = 0;
    for (const [productId, countedRaw] of Object.entries(counts)) {
      ...
      for (const unit of inStockUnits) {
        await supabase.from("product_units").update({ status: "pending_write_off", write_off_reason: "Reconciliation shortfall" }).eq("id", unit.id);
        await supabase.from("write_offs").insert({ user_id: userId, product_id: product.id, product_name: product.name, qty: 1, unit_code: unit.code, purchase_price: unit.purchase_price ?? product.purchase_price, reason: "Reconciliation shortfall", staff: activeStaff.name });
        unitsFlagged++;
      }
      if (inStockUnits.length > 0) shortfalls++;
    }
    loadAll();
    return { shortfalls, unitsFlagged, overages };
  };
  ...
  const confirmUnitSale = async () => {
    ...
    if (wanted > looseUnits) {
      packsToOpen = Math.ceil((wanted - looseUnits) / perPack);
      packQty -= packsToOpen;
      looseUnits += packsToOpen * perPack;
    }
    looseUnits -= wanted;

    // Mark exactly which sealed packs got opened — otherwise their codes
    // stay "in_stock" forever and could still be scanned as if they were
    // untouched whole packs even after their contents are long gone.
    if (packsToOpen > 0) {
      const toOpen = (unitsByProduct.get(product.id) || []).filter((u) => u.status === "in_stock").slice(0, packsToOpen);
      const nowIso = new Date().toISOString();
      for (const unit of toOpen) {
        await supabase.from("product_units").update({ status: "opened", opened_at: nowIso }).eq("id", unit.id);
      }
    }
    ...
    setUnitSaleProductId(""); setUnitSaleQty("");
    loadAll();
  };
  ...
  const NAV = [
    ...
    { id: "unit-sales", label: "Unit Sales", icon: Boxes },
  ...
  return (
    <div style={{ ...body, background: T.ink, minHeight: "100vh", color: T.ink, display: "flex", flexDirection: "column", fontSize: 14 }}>
      ...
      <div className="tb-sidebar" style={{ width: 190, flexShrink: 0, background: T.ink, color: T.paper, padding: "22px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div className="tb-sidebar-foot" style={{ marginTop: "auto", paddingTop: 18, borderTop: `1px solid ${T.inkSoft}` }}>
          ...
          {allowedScans.includes("sale") && unitSaleItems.length > 0 && (
            <button onClick={openUnitSale} className="tb-sellunits-quick-btn" style={{ width: "100%", marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "10px 10px", borderRadius: 4, border: `1px solid ${T.blueSoft}`, background: "transparent", color: T.paper, cursor: "pointer", fontWeight: 700, ...mono, fontSize: 12 }}>
        ...
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "26px 30px 60px" }}>
          {loading ? (
            ...
            <>
              ...
              {tab === "unit-sales" && (
                <UnitSales candidates={unitSaleCandidates} search={unitSaleSearch} setSearch={setUnitSaleSearch}
                  selected={unitSaleProduct} setSelected={setUnitSaleProductId} qty={unitSaleQty} setQty={setUnitSaleQty}
                  onConfirm={confirmUnitSale} currency={currency} history={history} />
              )}
              {tab === "refunds" && (
                <Refunds refunds={refunds} canScanRefund={allowedScans.includes("refund")} onScan={() => openScan("refund")}
                  canManageRefunds={activeStaff.role === "Admin"} onApproveRefund={approveRefund} onRejectRefund={rejectRefund}
                  looseUnitProducts={unitSaleItems} currency={currency} onRequestLooseRefund={requestLooseRefund} />
              )}
              {tab === "stock-health" && (
                <StockHealth products={products} unitCounts={unitCounts} history={history} currency={currency}
                  canWriteOff={allowedScans.includes("write-off")} onScanWriteOff={() => openScan("write-off")}
              ...
              {tab === "reports" && <Reports products={products} history={history} currency={currency} inventoryValue={inventoryValue} onExport={exportCSV} refunds={refunds} productUnits={productUnits} />}
      ...
      {scanOpen && (
        <ScanModal mode={scanMode} setMode={switchScanMode} onClose={closeScan} allowedScans={allowedScans}
          ...
          onRescan={rescan} cameraError={scanCameraError} setCameraError={setScanCameraError} onDetected={handleDetectedCode} onAddNew={addNewFromMissedCode}
          ...
          onConfirmStockIn={confirmStockInBatch} onConfirmStockOut={confirmStockOutBatch} currency={currency} />
      )}
...
/* ---------------------------------------------------------------
   Dashboard
------------------------------------------------------------------*/
function Dashboard({ settings, itemsSoldToday, stockAddedToday, todaysSales, inventoryValue, totalProductsAvailable, products, lowStock, outOfStock, expiringSoon, bestSellers, weeklyChart, currency, onScan, allowedScans, unitsSoldToday, hasUnitSaleItems, onSellUnits, onSelectProduct }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <form onSubmit={submit} style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "86vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        <div style={{ marginTop: 18, marginBottom: 6, paddingTop: 14, borderTop: `1px dashed ${T.paperDim}` }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: T.ink }}>Unit Sale</div>
          <div style={{ fontSize: 11, color: T.slate, marginTop: 1 }}>Turn this on for items you also sell loose, one piece at a time, out of a sealed pack.</div>
        </div>

        <div
          onClick={toggleUnitSale}
          style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: form.unit_sale_enabled ? 10 : 4, padding: "10px 12px", borderRadius: 4, background: form.unit_sale_enabled ? "#EEF2FE" : T.paper, border: `1px solid ${form.unit_sale_enabled ? T.blue : T.paperDim}` }}
        >
        ...
        {form.unit_sale_enabled && (
          <div style={{ background: T.paper, borderRadius: 4, padding: "12px 12px 2px", marginBottom: 12 }}>
            <Field label="Item name">
              <input value={form.name} disabled style={{ ...inputStyle, background: T.cream, color: T.slate }} />
            </Field>
...
/* ---------------------------------------------------------------
   ...
   accepts it here.
------------------------------------------------------------------*/
function Refunds({ refunds, canScanRefund, onScan, canManageRefunds, onApproveRefund, onRejectRefund, looseUnitProducts, currency, onRequestLooseRefund }) {
  ...
  const submitReturn = () => {
    if (!returnProductId || !returnQty || Number(returnQty) <= 0) return;
    onRequestLooseRefund(returnProductId, Number(returnQty));
    setReturnProductId(""); setReturnQty("");
  };

  return (
    <div>
      <PageHeader title="Refunds" sub="Items returned by a customer — scanned in (or logged, for loose units), then confirmed by an Admin before anything is added back to stock."
        ...
        <Panel title={`Pending Confirmation${pending.length ? ` (${pending.length})` : ""}`} icon={RotateCcw} iconColor={T.amber}>
          ...
          {pending.map((r) => (
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, padding: "10px 0", borderBottom: `1px solid ${T.paperDim}` }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{r.unit_code ? r.product_name : `${r.qty} \u00d7 ${r.product_name}`}</div>
                {r.unit_code ? (
                  <div style={{ ...mono, fontSize: 10.5, color: T.slateLight, marginTop: 1 }}>{r.unit_code}</div>
                ) : (
                  <div style={{ fontSize: 10.5, color: T.slateLight, marginTop: 1 }}>Loose units \u2014 no single code</div>
                )}
        ...
        <Panel title={`Awaiting Confirmation${pending.length ? ` (${pending.length})` : ""}`} icon={RotateCcw} iconColor={T.amber}>
          ...
          {pending.map((r) => (
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${T.paperDim}` }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{r.unit_code ? r.product_name : `${r.qty} \u00d7 ${r.product_name}`}</div>
                {r.unit_code && <div style={{ ...mono, fontSize: 10.5, color: T.slateLight, marginTop: 1 }}>{r.unit_code}</div>}
      ...
      {looseUnitProducts.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <Panel title="Return Loose Units">
            <div style={{ fontSize: 12, color: T.slate, marginBottom: 10 }}>
              For items sold individually (like a single tablet) that don't have their own code \u2014 log how many are being returned.
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
              <div style={{ flex: 2, minWidth: 160 }}>
                <Field label="Item">
                  <select value={returnProductId} onChange={(e) => setReturnProductId(e.target.value)} style={inputStyle}>
                    <option value="">Choose an item...</option>
                    {looseUnitProducts.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </Field>
              </div>
              <div style={{ flex: 1, minWidth: 90 }}>
                <Field label="Quantity">
                  <input type="number" min="1" value={returnQty} onChange={(e) => setReturnQty(e.target.value)} placeholder="0" style={inputStyle} />
                </Field>
              </div>
              <button onClick={submitReturn} disabled={!returnProductId || !returnQty} style={{ padding: "9px 16px", borderRadius: 4, border: "none", background: (returnProductId && returnQty) ? T.ink : T.slateLight, color: T.cream, cursor: (returnProductId && returnQty) ? "pointer" : "not-allowed", fontSize: 12.5, fontWeight: 600, marginBottom: 2 }}>Log Return</button>
            </div>
          </Panel>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <Panel title="Reconcile" icon={ClipboardCheck} iconColor={T.blue}>
        <div style={{ fontSize: 12, color: T.slate, marginBottom: 12 }}>
          Physically count what's on the shelf and enter it below — leave anything you haven't counted this session blank. Counting fewer than expected flags the shortfall as a write-off awaiting your confirmation above; counting more just gets flagged, since new stock needs its own code first (see Labels).
        </div>
        ...
        {summary && (
          <div style={{ marginTop: 12, fontSize: 12.5, color: T.slate, background: T.paper, borderRadius: 4, padding: "10px 12px" }}>
            {summary.shortfalls === 0 && summary.overages === 0 ? "Everything matched — nothing to reconcile." : (
              <>
                {summary.shortfalls > 0 && <div>{summary.unitsFlagged} unit{summary.unitsFlagged === 1 ? "" : "s"} flagged for write-off across {summary.shortfalls} item{summary.shortfalls === 1 ? "" : "s"} that came up short — see Pending Write Offs above to confirm.</div>}
...
function Reports({ products, history, currency, inventoryValue, onExport, refunds, productUnits }) {
  const grossRevenue = history.filter((h) => h.type === "sale" || h.type === "unit-sale").reduce((s, h) => s + h.qty * Number(h.unit_price), 0);
  // Refunds reduce revenue whether the item was restocked or written off —
  // the money genuinely left the till either way.
  const refundValue = refunds.filter((r) => r.status === "approved").reduce((s, r) => s + Number(r.sale_price || 0), 0);
  const totalSaleRevenue = grossRevenue - refundValue;
  const writeOffValue = history.filter((h) => h.type === "write-off").reduce((s, h) => s + h.qty * Number(h.unit_price || 0), 0);
  ...
  const valueOf = (p) => {
    const allUnits = (productUnits || []).filter((u) => u.product_id === p.id);
    const sealed = allUnits.filter((u) => u.status === "in_stock" || u.status === "pending_write_off");
    let total = sealed.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0);
    const gap = Math.max(0, p.qty - sealed.length);
    total += gap * Number(p.purchase_price);
    if (p.unit_sale_enabled && p.loose_units > 0) {
      const perPack = Number(p.unit_qty_per_pack) || 1;
      const openedUnits = allUnits.filter((u) => u.status === "opened");
      const avgCost = openedUnits.length > 0
        ? openedUnits.reduce((s, u) => s + Number(u.purchase_price ?? p.purchase_price), 0) / openedUnits.length
        : Number(p.purchase_price);
      total += (p.loose_units / perPack) * avgCost;
  ...
  const byCategory = CATEGORIES.map((c) => ({
    ...
    value: products.filter((p) => p.category === c).reduce((s, p) => s + valueOf(p), 0),
  ...
  return (
    <div>
      ...
      <div className="tb-stat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10, marginBottom: 20 }}>
        <StatStub label="Refunded (net of revenue above)" value={`${currency}${refundValue.toLocaleString()}`} accent={T.amber} />
        <StatStub label="Written Off (damaged/expired/lost)" value={`${currency}${writeOffValue.toLocaleString()}`} accent={T.amber} />
      </div>
          ...
          <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${T.paperDim}`, flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {isOwner && !s.auth_user_id && (
                <button onClick={() => onSetupLogin(s)} style={{ padding: "6px 10px", borderRadius: 4, border: `1px solid ${T.blue}`, background: "transparent", color: T.blue, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Set up device login</button>
              )}
...
function ScanModal({
  mode, setMode, onClose, allowedScans,
  ...
  pile, onRemoveFromPile, onDiscardPile, onConfirmStockIn, onConfirmStockOut, currency,
}) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 55, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", width: 420, maxWidth: "94vw", maxHeight: "92vh", overflow: "auto", background: T.cream, borderRadius: 6, padding: 22 }}>
        ...
        {cameraStage === "off" && !cameraError && !missCode && !writeOffPending && (
        ...
        {piles && (
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${T.paperDim}` }}>
            <button
              ...
              style={{ width: "100%", padding: "12px", borderRadius: 4, border: "none", background: pile.length ? (mode === "stock-in" ? T.green : T.stamp) : T.slateLight, color: "#fff", cursor: pile.length ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 700, ...mono }}
            >
              {mode === "stock-in" ? `ACCEPT${pile.length ? ` ${pile.length} ITEM${pile.length === 1 ? "" : "S"}` : ""}` : `MARK ${pile.length || ""} SOLD \u0026 PRINT RECEIPT`}
            </button>
          </div>
        )}
      </div>
...
------------------------------------------------------------------*/
function UnitSales({ candidates, search, setSearch, selected, setSelected, qty, setQty, onConfirm, currency, history }) {
  ...
  const recentUnitSales = useMemo(() => history.filter((h) => h.type === "unit-sale").slice(0, 25), [history]);

  return (
    <div>
      <PageHeader title="Unit Sales" sub="Search a stocked item that's set up for unit sale, choose a quantity, and mark it sold — the pack it comes from is adjusted automatically." />
      <div className="tb-panel-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
        <Panel title="Sell a Unit" icon={Boxes} iconColor={T.blue}>
          {!selected ? (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: T.cream, borderRadius: 4, padding: "8px 10px", marginBottom: 10 }}>
                <Search size={13} color={T.slate} />
              ...
              <div style={{ maxHeight: 320, overflow: "auto" }}>
                ...
                {candidates.length === 0 && <Empty text="No items are set up for unit sale yet — enable it when adding an item in Labels." />}
              </div>
            </>
          ) : (
            <div>
              <div style={{ background: T.cream, borderRadius: 4, padding: 14, marginBottom: 12 }}>
        ...
        <Panel title="Recent Unit Sales">
          {recentUnitSales.length === 0 && <Empty text="No unit sales recorded yet." />}
          {recentUnitSales.map((h) => (
            <div key={h.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${T.paperDim}` }}>
              <div>
                <div style={{ fontSize: 13 }}>{h.product_name}</div>
                <div style={{ fontSize: 11, color: T.slate }}>{h.occurred_on} · {h.staff}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ ...mono, fontSize: 12.5, fontWeight: 700 }}>{h.qty} sold</div>
                <div style={{ ...mono, fontSize: 11, color: T.slate }}>{currency}{(h.qty * Number(h.unit_price)).toLocaleString()}</div>
              </div>
            </div>
          ))}
        </Panel>
