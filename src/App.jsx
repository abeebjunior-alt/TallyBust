...
const RESPONSIVE_CSS = `
  * { box-sizing: border-box; }
  @media (max-width: 760px) {
    ...
    .tb-table-scroll > div { min-width: 560px; }
  }
`;
...
function AddProductModal({ onAdd, onClose, initialSku }) {
  const [form, setForm] = useState({ name: "", category: "Medicine", sku: initialSku || "", qty: 0, min_stock: 5, purchase_price: 0, selling_price: 0, supplier: "", expiry: "", batch: "" });
  const set = (k) => (e) => setForm({ ...form, [k]: e.target.value });
  const submit = (e) => {
    e.preventDefault();
    onAdd({ ...form, qty: Number(form.qty), min_stock: Number(form.min_stock), purchase_price: Number(form.purchase_price), selling_price: Number(form.selling_price), expiry: form.expiry || null });
  };
  return (
