...
function TallyBust({ userId, userEmail, initialActiveStaff }) {
  ...
  const handleDetectedCode = async (rawCode) => {
    ...
    if (scanMode === "refund") {
      ...
      const { error: e2 } = await supabase.from("refunds").insert({ user_id: userId, product_id: product.id, product_name: product.name, qty: 1, unit_code: unit.code, sale_price: salePrice, staff: activeStaff.name });
