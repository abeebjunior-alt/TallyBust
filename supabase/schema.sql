...
-- ---------------------------------------------------------------------
-- refunds: a customer return recorded by any staff who can make a sale.
-- Stays 'pending' — and does NOT touch stock — until the business Admin
-- approves it, at which point the app adds the quantity back to stock
-- (recorded as a normal stock-in) and marks it 'approved'.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- stock_units: one row per PHYSICAL printed label (not per product).
-- Each label's QR encodes this row's id, so every unit is individually
-- trackable and a label can't be scanned in as stock twice.
-- ---------------------------------------------------------------------
